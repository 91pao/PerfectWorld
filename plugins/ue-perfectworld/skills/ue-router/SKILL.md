---
name: ue-router
description: Route Unreal Engine tasks across planning, read-only guidance, implementation, investigation, review, and Blueprint/editor workflows. Use for UE C++, Blueprint, gameplay, UI, networking, assets/data, editor, build, configuration, and architecture work.
---
...
Use this router only to choose the workflow, then follow the chosen workflow's rules.
...
- Read-only guided implementation, "不要改文件", "我自己写", "把完整代码给我", or ambiguous code-writing requests: use `ue-draft`
...
Read `../../references/ue-core-rules.md` for every routed UE task.
Read `../../references/ue-client-server-boundary-rules.md` for every routed UE task.

If the user's wording is ambiguous about file edits, route to `ue-draft` or `ue-investigate` and keep the workspace read-only.

Follow the routed skill's own required reference list instead of duplicating it here.
