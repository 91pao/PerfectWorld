...
name: landing-report
version: 0.1.0
description: Read-only queue dashboard for workspace-aware ship. (perfectworld)
triggers:
  - landing report
  - version queue
  - ship queue
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"landing-report","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"landing-report","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"landing-report","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
`perfectworld-ship` would land in. This skill is a read-only call into the same
`bin/perfectworld-next-version` utility `perfectworld-ship` uses, but with nothing mutating.
...
Same detection as other perfectworld skills.

```bash
BASE_BRANCH=$(gh pr view --json baseRefName -q .baseRefName 2>perfectworld-dev/null || \
              gh repo view --json defaultBranchRef -q .defaultBranchRef.name 2>perfectworld-dev/null || \
              echo main)
...
```bash
CURRENT_VERSION=$(cat VERSION 2>perfectworld-dev/null | tr -d '[:space:]' || echo "0.0.0.0")
git fetch origin "$BASE_BRANCH" --quiet 2>perfectworld-dev/null || true
BASE_VERSION=$(git show "origin/$BASE_BRANCH:VERSION" 2>perfectworld-dev/null | tr -d '[:space:]' || echo "$CURRENT_VERSION")
...
for LEVEL in micro patch minor major; do
  bun run bin/perfectworld-next-version \
    --base "$BASE_BRANCH" \
    --bump "$LEVEL" \
    --current-version "$BASE_VERSION" \
    > "perfectworld-tmp/landing-$LEVEL.json" 2>perfectworld-dev/null || echo '{"offline":true}' > "perfectworld-tmp/landing-$LEVEL.json"
done
```
...
If you ran perfectworld-ship right now, you'd claim:
...
1. **If there are collisions in the queue** (two open PRs claim the same version):
   ...
   to rerun perfectworld-ship to pick up the next free slot."
...
3. **If everything looks clean:**
   "Queue is clean. Next perfectworld-ship will claim a slot without conflict."
