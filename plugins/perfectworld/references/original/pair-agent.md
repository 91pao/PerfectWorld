...
name: pair-agent
version: 0.1.0
description: Pair a remote AI agent with your browser. (perfectworld)
triggers:
  - pair with agent
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"pair-agent","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"pair-agent","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"pair-agent","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-pair-agent — Share Your Browser With Another AI Agent
...
Your perfectworld browser runs a local HTTP server. This skill creates a one-time setup key,
...
```bash
$B status 2>perfectworld-dev/null
```
...
```bash
which ngrok 2>perfectworld-dev/null && echo "NGROK_INSTALLED" || echo "NGROK_NOT_INSTALLED"
ngrok config check 2>perfectworld-dev/null && echo "NGROK_AUTHED" || echo "NGROK_NOT_AUTHED"
```
...
1. Go to https:/perfectworld-dashboard.ngrok.com/get-started/your-authtoken
...
1. Go to https:/perfectworld-ngrok.com and sign up (free tier works)
2. Install ngrok:
   - macOS: `brew install ngrok`
   - Linux: `snap install ngrok` or download from ngrok.com/download
3. Auth it: `ngrok config add-authtoken YOUR_TOKEN`
   (get your token from https:/perfectworld-dashboard.ngrok.com/get-started/your-authtoken)
4. Come back here and run `perfectworld-pair-agent` again."
...
side panel if you have PerfectWorld Browser open."
...
**"Token expired"** — The 24-hour session expired. Run `perfectworld-pair-agent` again to
...
credentials are written to `~/.openclaw/skills/perfectworld/browse-remote.json`.
...
to `~/.codex/skills/perfectworld/browse-remote.json`.
...
`~/.cursor/skills/perfectworld/browse-remote.json`.
