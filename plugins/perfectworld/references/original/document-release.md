...
preamble-tier: 2
version: 1.0.0
description: Post-ship documentation update. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"document-release","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"document-release","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
You are running the `perfectworld-document-release` workflow. This runs **after `perfectworld-ship`** (code committed, PR
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.perfectworld/*" -not -path "./.context/*" | sort
```
...
Coverage map:
  ...
  perfectworld-new-skill       ✅ AGENTS.md  ❌        ❌          ❌
...
When significant gaps are found, suggest running `perfectworld-document-generate` to fill them.

---

> **STOP.** Before auditing each doc file and applying updates, polishing CHANGELOG voice, checking cross-doc consistency, cleaning up TODOS, the VERSION bump, and committing (Steps 2-9, after the coverage map in Step 1.5), Read `$PERFECTWORLD_SOURCE/document-release/sections/release-body.md` and execute it
...
- **Discoverability matters.** Every doc file should be reachable from README or AGENTS.md or repository guidance.
- **Coverage map informs, never generates.** The Diataxis coverage map flags gaps for the PR body
  ...
  are found, suggest `perfectworld-document-generate` as the follow-up skill.
