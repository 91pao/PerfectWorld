...
preamble-tier: 4
version: 2.0.0
description: Systematically QA test a web application and fix bugs found. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-qa: Test → Fix → Verify
...
|-----------|---------|-----------------:|
| Target URL | (auto-detect or required) | `https:/perfectworld-myapp.com`, `http:/perfectworld-localhost:3000` |
| Tier | Standard | `--quick`, `--exhaustive` |
| Mode | full | `--regression .perfectworld/qa-reports/baseline.json` |
| Output dir | `.perfectworld/qa-reports/` | `Output to perfectworld-tmp/qa` |
...
"Your working tree has uncommitted changes. perfectworld-qa needs a clean tree so each bug fix gets its own atomic commit."
...
```bash
mkdir -p .perfectworld/qa-reports/screenshots
```
...
if [ "$_CROSS_PROJ" = "true" ]; then
  $PERFECTWORLD_SOURCE/bin/perfectworld-learnings-search --limit 10 --query "qa testing bug regression flake fixture" --cross-project 2>perfectworld-dev/null || true
else
  $PERFECTWORLD_SOURCE/bin/perfectworld-learnings-search --limit 10 --query "qa testing bug regression flake fixture" 2>perfectworld-dev/null || true
fi
```
...
1. **Project-scoped test plans:** Check `~/.perfectworld/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>perfectworld-dev/null || true  # zsh compat
   eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
   ls -t ~/.perfectworld/projects/$SLUG/*-test-plan-*.md 2>perfectworld-dev/null | head -1
   ```
2. **Conversation context:** Check if a prior `perfectworld-plan-eng-review` or `perfectworld-plan-ceo-review` produced test plan output in this conversation
...
This is the **primary mode** for developers verifying their work. When the user says `perfectworld-qa` without a URL and the repo is on a feature branch, automatically:
...
2. **Identify affected pages/routes** from the changed files:
   ...
   - API endpoints → test them directly with `$B js "await fetch('perfectworld-api/...')"`
   - Static pages (markdown, HTML) → navigate to them directly

   **If no obvious pages/routes are identified from the diff:** Do not skip browser testing. The user invoked perfectworld-qa because they want browser-based verification. Fall back to Quick mode — navigate to the homepage, follow the top 5 navigation targets, check console for errors, and test any interactive elements found. Backend, config, and infrastructure changes affect app behavior — always verify the app still works.

3. **Detect the running app** — check common local dev ports:
   ```bash
   $B goto http:/perfectworld-localhost:3000 2>perfectworld-dev/null && echo "Found app on :3000" || \
   $B goto http:/perfectworld-localhost:4000 2>perfectworld-dev/null && echo "Found app on :4000" || \
   $B goto http:/perfectworld-localhost:8080 2>perfectworld-dev/null && echo "Found app on :8080"
   ```
...
- Test REST API endpoints (`perfectworld-wp-json/`)
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-search --query "<your-keyword>" --limit 5 2>perfectworld-dev/null || true
```
...
- Include full attribution comment:
  ...
  // Found by perfectworld-qa on {YYYY-MM-DD}
  // Report: .perfectworld/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.perfectworld/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)" && mkdir -p ~/.perfectworld/projects/$SLUG
```
Write to `~/.perfectworld/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
...
2. **Fixed bugs that were in TODOS.md** → annotate with "Fixed by perfectworld-qa on {branch}, {date}"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-log '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
