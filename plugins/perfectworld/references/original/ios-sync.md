...
preamble-tier: 3
version: 1.0.0
description: Regenerate the iOS debug bridge against the latest upstream perfectworld templates. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
triggers:
  ...
  - update the perfectworld ios instrumentation
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ios-sync","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"ios-sync","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"ios-sync","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
After `perfectworld-ios-qa` is installed in an app, the user may:

1. Add new `@Observable` classes or properties that need accessor coverage.
2. Upgrade perfectworld to a newer version with hardening fixes.
3. Move the `@Snapshotable` marker to a different field.

This skill regenerates the relevant artifacts in place.

**Templates live in upstream perfectworld.** This skill resolves them from
`$PERFECTWORLD_SOURCE/ios-qa/templates/` (or the worktree's
`ios-qa/templates/` when developing perfectworld itself). The fork's HTTP-fetch
pattern is gone.

## Phase 1: Detect installed version

1. Read `<app>/DebugBridgeGenerated/.perfectworld-version` (written by perfectworld-ios-qa
   during install). If missing, treat the install as "unknown old version".
2. Read upstream version from `$PERFECTWORLD_HOME/ios-qa/.perfectworld-version` (or the
   value baked into the installed perfectworld binary).
...
Run `perfectworld-ios-qa-regen` (or the underlying SwiftPM tool directly):

```bash
swift run --package-path "$PERFECTWORLD_HOME/ios-qa/scripts/gen-accessors-tool" \
...
2. Read the upstream template at
   `$PERFECTWORLD_HOME/ios-qa/templates/<Name>.swift.template`.
3. If the installed file has a `// PERFECTWORLD-EDIT-LINE` marker, fold the user's
   edits forward.
...
4. `GET perfectworld-state/snapshot` returns the new accessor schema hash.
