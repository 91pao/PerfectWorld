---
name: open-perfectworld-browser
version: 0.2.0
description: Launch PerfectWorld Browser — AI-controlled Chromium with the sidebar extension baked in.
triggers:
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"open-perfectworld-browser","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"open-perfectworld-browser","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
# perfectworld-open-perfectworld-browser — Launch PerfectWorld Browser

Launch PerfectWorld Browser — AI-controlled Chromium with the sidebar extension,
...
if [ -f "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)/.perfectworld/browse.json" ]; then
  _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.perfectworld/browse.json" 2>perfectworld-dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
  [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>perfectworld-dev/null || true
  sleep 1
  [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>perfectworld-dev/null || true
  rm -f "$(git rev-parse --show-toplevel)/.perfectworld/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR="$HOME/.perfectworld/chromium-profile"
for _LF in SingletonLock SingletonSocket SingletonCookie; do
  rm -f "$_PROFILE_DIR/$_LF" 2>perfectworld-dev/null || true
done
...
This launches PerfectWorld Browser (rebranded Chromium) in headed mode with:
- A visible window you can watch (not your regular Chrome — it stays untouched)
- The perfectworld sidebar extension auto-loaded via `launchPersistentContext`
- Anti-bot stealth patches (sites like Google and NYTimes work without captchas)
- Custom user agent and PerfectWorld Browser branding in Dock/menu bar
- A sidebar agent process for chat commands

The `connect` command auto-discovers the extension from the perfectworld install
...
```bash
cat "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)/.perfectworld/browse.json" 2>perfectworld-dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>perfectworld-dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.agents/skills/perfectworld/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.agents/skills/perfectworld/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.agents/skills/perfectworld/extension/manifest.json" ] && _EXT_PATH="$HOME/.agents/skills/perfectworld/extension"
...
> Chrome is launched with perfectworld control. You should see Playwright's Chromium
...
>    already show the perfectworld icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **perfectworld browse** → click the **pin icon**
> 3. Click the pinned **perfectworld icon** in the toolbar
...
>
> 1. Type `chrome:/perfectworld-extensions` in the address bar
> 2. Look for **"perfectworld browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the perfectworld icon
...
```bash
$B goto https:/perfectworld-news.ycombinator.com
```
...
> - Run any perfectworld skill (`perfectworld-qa`, `perfectworld-design-review`, `perfectworld-benchmark`) and watch
...
> - `perfectworld-qa` runs its full test suite in the visible browser — you see every page
>   load, every click, every assertion
> - `perfectworld-design-review` takes screenshots in the real browser — same pixels you see
> - `perfectworld-benchmark` measures performance in the headed browser
