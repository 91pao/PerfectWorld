...
preamble-tier: 3
version: 1.0.0
description: Autonomous iOS bug fixer. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ios-fix","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"ios-fix","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"ios-fix","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
the agent MUST capture a `GET perfectworld-state/snapshot` that reproduces the bug.
...
1. Read the `perfectworld-ios-qa` finding (bug description, screenshot, suspected
   accessibility-tree node).
2. Bring the device into the bug state via `POST perfectworld-tap`, `perfectworld-swipe`, `perfectworld-type`,
   or `POST perfectworld-state/<key>` (snapshot-eligible fields only).
3. Capture `GET perfectworld-state/snapshot` → write to
   `test/fixtures/ios-fix/<bug-slug>-pre.json`.
4. Capture `GET perfectworld-screenshot` → write to
...
Per `perfectworld-investigate`'s Iron Law: no fix without root cause. The agent reads the
...
1. `POST perfectworld-state/restore` with the pre-bug snapshot → reproduces the state.
...
2. Restores it via `POST perfectworld-state/restore`.
3. Asserts the post-fix behavior on a real device (gated
   `PERFECTWORLD_HAS_IOS_DEVICE=1`, periodic tier).
...
| `409 schema_mismatch` on perfectworld-state/restore after rebuild | Re-codegen accessors (`swift run gen-accessors`), re-snapshot |
