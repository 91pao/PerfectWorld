...
preamble-tier: 4
version: 1.0.0
description: Land and deploy workflow. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"land-and-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
git remote get-url origin 2>perfectworld-dev/null
```

- If the URL contains "github.com" → platform is **GitHub**
- If the URL contains "gitlab" → platform is **GitLab**
- Otherwise, check CLI availability:
  - `gh auth status 2>perfectworld-dev/null` succeeds → platform is **GitHub** (covers GitHub Enterprise)
  - `glab auth status 2>perfectworld-dev/null` succeeds → platform is **GitLab** (covers self-hosted)
...
**If GitLab:**
1. `glab mr view -F json 2>perfectworld-dev/null` and extract the `target_branch` field — if succeeds, use it
2. `glab repo view -F json 2>perfectworld-dev/null` and extract the `default_branch` field — if succeeds, use it

**Git-native fallback (if unknown platform, or CLI commands fail):**
1. `git symbolic-ref refs/remotes/origin/HEAD 2>perfectworld-dev/null | sed 's|refs/remotes/origin/||'`
2. If that fails: `git rev-parse --verify origin/main 2>perfectworld-dev/null` → use `main`
3. If that fails: `git rev-parse --verify origin/master 2>perfectworld-dev/null` → use `master`
...
**If the platform detected above is GitLab or unknown:** STOP with: "GitLab support for perfectworld-land-and-deploy is not yet implemented. Run `perfectworld-ship` to create the MR, then merge manually via the GitLab web UI." Do not proceed.

# perfectworld-land-and-deploy — Merge, Deploy, Verify

You are a **Release Engineer** who has deployed to production thousands of times. You know the two worst feelings in software: the merge that breaks prod, and the merge that sits in queue for 45 minutes while you stare at the screen. Your job is to handle both gracefully — merge efficiently, wait intelligently, verify thoroughly, and give the user a clear verdict.

This skill picks up where `perfectworld-ship` left off. `perfectworld-ship` creates the PR. You merge it, wait for deploy, and verify production.

## User-invocable
When the user types `perfectworld-land-and-deploy`, run this skill.

## Arguments
- `perfectworld-land-and-deploy` — auto-detect PR from current branch, no post-deploy URL
- `perfectworld-land-and-deploy <url>` — auto-detect PR, verify deploy at this URL
- `perfectworld-land-and-deploy #123` — specific PR number
- `perfectworld-land-and-deploy #123 <url>` — specific PR + verification URL

## Non-interactive philosophy (like perfectworld-ship) — with one critical gate

This is a **mostly automated** workflow. Do NOT ask for confirmation at any step except
the ones listed below. The user said `perfectworld-land-and-deploy` which means DO IT — but verify
readiness first.
...
gh auth status
```
If not authenticated, **STOP**: "I need GitHub CLI access to merge your PR. Run `gh auth login` to connect, then try `perfectworld-land-and-deploy` again."
...
5. Validate the PR state:
   - If no PR exists: **STOP.** "No PR found for this branch. Run `perfectworld-ship` first to create a PR, then come back here to land and deploy it."
   - If `state` is `MERGED`: "This PR is already merged — nothing to deploy. If you need to verify the deploy, run `perfectworld-canary <url>` instead."
...
Check whether this project has been through a successful `perfectworld-land-and-deploy` before,
and whether the deploy configuration has changed since then:

```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
if [ ! -f ~/.perfectworld/projects/$SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat ~/.perfectworld/projects/$SLUG/land-deploy-confirmed 2>perfectworld-dev/null)
  CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' AGENTS.md or repository guidance 2>perfectworld-dev/null | shasum -a 256 | cut -d' ' -f1)
  # Also hash workflow files that affect deploy behavior
  WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>perfectworld-dev/null | xargs cat 2>perfectworld-dev/null | shasum -a 256 | cut -d' ' -f1)
...
**If FIRST_RUN:** This is the first time `perfectworld-land-and-deploy` is running for this project. Before doing anything irreversible, show the user exactly what will happen. This is a dry run — explain, validate, and confirm.
...
```bash
# Check for persisted deploy config in AGENTS.md or repository guidance
DEPLOY_CONFIG=$(grep -A 20 "## Deploy Configuration" AGENTS.md or repository guidance 2>perfectworld-dev/null || echo "NO_CONFIG")
echo "$DEPLOY_CONFIG"
...
for f in $(find .github/workflows -maxdepth 1 \( -name '*.yml' -o -name '*.yaml' \) 2>perfectworld-dev/null); do
  [ -f "$f" ] && grep -qiE "deploy|release|production|cd" "$f" 2>perfectworld-dev/null && echo "DEPLOY_WORKFLOW:$f"
  [ -f "$f" ] && grep -qiE "staging" "$f" 2>perfectworld-dev/null && echo "STAGING_WORKFLOW:$f"
done
```

If `PERSISTED_PLATFORM` and `PERSISTED_URL` were found in AGENTS.md or repository guidance, use them directly
and skip manual detection. If no persisted config exists, use the auto-detected platform
to guide deploy verification. If nothing is detected, ask the user via AskUserQuestion
in the decision tree below.

If you want to persist deploy settings for future runs, suggest the user run `perfectworld-setup-deploy`.

Parse the output and record: the detected platform, production URL, deploy workflow (if any),
and any persisted config from AGENTS.md or repository guidance.
...
# Fly.io: fly status --app {app} 2>perfectworld-dev/null
# Heroku: heroku releases --app {app} -n 1 2>perfectworld-dev/null
# Vercel: vercel ls 2>perfectworld-dev/null | head -3

# Test production URL reachability
# curl -sf {production-url} -o perfectworld-dev/null -w "%{http_code}" 2>perfectworld-dev/null
```
...
1. **AGENTS.md or repository guidance persisted config:** Check for a staging URL in the Deploy Configuration section:
```bash
grep -i "staging" AGENTS.md or repository guidance 2>perfectworld-dev/null | head -3
```
...
```bash
gh pr checks --json name,targetUrl 2>perfectworld-dev/null | head -20
```
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-review-read 2>perfectworld-dev/null
```
...
- **RECOMMENDATION:** Choose A if all validations passed. Choose B if there are issues to fix. Choose C to run perfectworld-setup-deploy for a more thorough configuration.
...
- C) I want to configure this more carefully first (runs perfectworld-setup-deploy) (Completeness: 10/10)

**If A:** Tell the user: "Great — I've saved this configuration. Next time you run `perfectworld-land-and-deploy`, I'll skip the dry run and go straight to readiness checks. If your deploy setup changes (new platform, different workflows, updated URLs), I'll automatically re-run the dry run to make sure I still have it right."

Save the deploy config fingerprint so we can detect future changes:
```bash
mkdir -p ~/.perfectworld/projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' AGENTS.md or repository guidance 2>perfectworld-dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>perfectworld-dev/null | xargs cat 2>perfectworld-dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > ~/.perfectworld/projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.

**If B:** **STOP.** "Tell me what's different about your setup and I'll adjust. You can also run `perfectworld-setup-deploy` to walk through the full configuration."

**If C:** **STOP.** "Running `perfectworld-setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to AGENTS.md or repository guidance so I'll know exactly what to do next time. Run `perfectworld-land-and-deploy` again when that's done."
...
```
If `CONFLICTING`: **STOP.** "This PR has merge conflicts with the base branch. Resolve the conflicts and push, then run `perfectworld-land-and-deploy` again."
...
Before gathering readiness evidence, verify that the VERSION this PR claims is still the next free slot. A sibling workspace may have shipped and landed since `perfectworld-ship` ran, leaving this PR's VERSION stale.

```bash
BRANCH_VERSION=$(git show HEAD:VERSION 2>perfectworld-dev/null | tr -d '\r\n[:space:]' || echo "")
BASE_BRANCH=$(gh pr view --json baseRefName -q .baseRefName 2>perfectworld-dev/null || echo main)
BASE_VERSION=$(git show origin/$BASE_BRANCH:VERSION 2>perfectworld-dev/null | tr -d '\r\n[:space:]' || echo "")
...
QUEUE_JSON=$(bun run bin/perfectworld-next-version \
  --base "$BASE_BRANCH" \
  --bump patch \
  --current-version "$BASE_VERSION" 2>perfectworld-dev/null || echo '{"offline":true}')
...
3. If drift is detected (a PR landed ahead of us and `BRANCH_VERSION < NEXT_SLOT`): **STOP** and print exactly:
   ...
   ⚠ VERSION drift detected.
     ...
     Next free slot:  v<NEXT_SLOT>   (queue moved since last perfectworld-ship)

   Rerun perfectworld-ship from the feature branch to reconcile. perfectworld-ship's ALREADY_BUMPED
   ...
   Exit non-zero. Do NOT auto-bump from `perfectworld-land-and-deploy` — rerunning `perfectworld-ship` is the clean path (it already handles VERSION + package.json + CHANGELOG header + PR title atomically via Step 12 ALREADY_BUMPED detection).
...
- B) Stop and run a full `perfectworld-review` first — deeper analysis, more thorough (Completeness: 10/10)
...
```bash
cat $PERFECTWORLD_SOURCE/review/checklist.md 2>perfectworld-dev/null || echo "Checklist not found"
```
Apply each checklist item to the current diff. This is the same quick review that `perfectworld-ship`
...
and tell the user: "I found and fixed a few issues during the review. The fixes are committed — run `perfectworld-land-and-deploy` again to pick them up and continue where we left off."

**If no issues found:** Tell the user: "Review checklist passed — no issues found in the diff."

**If B:** **STOP.** "Good call — run `perfectworld-review` for a thorough pre-landing review. When that's done, run `perfectworld-land-and-deploy` again and I'll pick up right where we left off."
...
Read AGENTS.md or repository guidance to find the project's test command. If not specified, use `bun test`.
...
```bash
setopt +o nomatch 2>perfectworld-dev/null || true  # zsh compat
ls -t ~/.perfectworld-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>perfectworld-dev/null | head -20
```
...
```bash
setopt +o nomatch 2>perfectworld-dev/null || true  # zsh compat
ls -t ~/.perfectworld-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>perfectworld-dev/null | head -5
```
...
```bash
git log --oneline $(gh pr view --json baseRefName -q .baseRefName 2>perfectworld-dev/null || echo main)..HEAD | head -20
```
...
```bash
git log --oneline --all-match --grep="docs:" $(gh pr view --json baseRefName -q .baseRefName 2>perfectworld-dev/null || echo main)..HEAD | head -5
```

Also check if key doc files were modified:
```bash
git diff --name-only $(gh pr view --json baseRefName -q .baseRefName 2>perfectworld-dev/null || echo main)...HEAD -- README.md CHANGELOG.md ARCHITECTURE.md CONTRIBUTING.md AGENTS.md or repository guidance VERSION
```

If CHANGELOG.md and VERSION were NOT modified on this branch and the diff includes
new features (new files, new commands, new skills): **WARNING — perfectworld-document-release
...
- If reviews are stale: "Run `perfectworld-review` or `perfectworld-autoplan` to review the current code, then `perfectworld-land-and-deploy` again."
- If E2E not run: "Run your E2E tests to make sure nothing is broken, then come back."
- If docs not updated: "Run `perfectworld-document-release` to update CHANGELOG and docs."
...
Then run `perfectworld-diff-scope` to classify the changes:

```bash
eval $($PERFECTWORLD_SOURCE/bin/perfectworld-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>perfectworld-dev/null || echo main) 2>perfectworld-dev/null)
...
If staging was detected in Step 1.5c (or from AGENTS.md or repository guidance deploy config), and the changes
...
Then tell the user: "Staging looks good. When you're ready for production, run `perfectworld-land-and-deploy` again."
**STOP.** The user can re-run `perfectworld-land-and-deploy` later for production.
...
If a deploy status command was configured in AGENTS.md or repository guidance (e.g., `fly status --app myapp`), use it instead of or in addition to GitHub Actions polling.

**Fly.io:** After merge, Fly deploys via GitHub Actions or `fly deploy`. Check with:
```bash
fly status --app {app} 2>perfectworld-dev/null
```
...
```bash
heroku releases --app {app} -n 1 2>perfectworld-dev/null
```
...
If AGENTS.md or repository guidance has a custom deploy status command in the "Custom deploy hooks" section, run that command and check its exit code.
...
```bash
$B snapshot -i -a -o ".perfectworld/deploy-reports/post-deploy.png"
```
...
```bash
mkdir -p .perfectworld/deploy-reports
```
...
Save report to `.perfectworld/deploy-reports/{date}-pr{number}-deploy.md`.
...
- If a production URL was verified: "Want extended monitoring? Run `perfectworld-canary <url>` to watch the site for the next 10 minutes."
- If performance data was collected: "Want a deeper performance analysis? Run `perfectworld-benchmark <url>`."
- "Need to update docs? Run `perfectworld-document-release` to sync README, CHANGELOG, and other docs with what you just shipped."
...
- **Single-pass verification, not continuous monitoring.** `perfectworld-land-and-deploy` checks once. `perfectworld-canary` does the extended monitoring loop.
