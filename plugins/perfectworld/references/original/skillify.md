...
name: skillify
version: 1.0.0
description: Codify the most recent successful perfectworld-scrape flow into a permanent browser-skill on disk. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"skillify","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"skillify","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"skillify","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-skillify — codify the last scrape into a permanent skill

The productivity multiplier. `perfectworld-scrape` discovered how to pull the data;
`perfectworld-skillify` writes it as deterministic Playwright-via-`browse-client`
code so the next `perfectworld-scrape` call on the same intent runs in ~200ms.

Without this command, `perfectworld-scrape` is a slow wrapper around `$B`. With it,
...
for the most recent `perfectworld-scrape` invocation that:
...
> "No recent perfectworld-scrape result found in this conversation. Run perfectworld-scrape
> <intent> first, then say perfectworld-skillify."

Stop. Do not synthesize from chat fragments. Do not synthesize from a
match-path perfectworld-scrape result (matched skills are already codified — there's
nothing to skillify).
...
> "The last successful perfectworld-scrape was '<intent line>' a few turns back.
> Skillify that one?"
...
- 3–5 trigger phrases the agent should match against in future `perfectworld-scrape`
...
```
D<N> — Skill name + tier
Project/branch/task: codifying perfectworld-scrape "<intent>" as a browser-skill.
...
A) Keep "<proposed-name>" at global tier — ~/.perfectworld/browser-skills/<proposed-name>/  (recommended)
B) Keep "<proposed-name>" but at project tier — <project>/.perfectworld/browser-skills/<proposed-name>/
...
```bash
$B goto "<TARGET_URL>"
$B html > perfectworld-tmp/skillify-fixture-$$.html
```
...
The canonical SDK lives at `<perfectworld-install>perfectworld-browse/src/browse-client.ts`.
The bundled-skill loader walks the install tree to find it; mirror that.

Resolve the perfectworld install dir. Two reliable signals (in order):

1. The bundled `hackernews-frontpage` skill — look at its tier path from
   `$B skill list` (the `bundled` row). The skill dir is
   `<perfectworld-install>perfectworld-browser-skills/hackernews-frontpage/`, so the install
   dir is two `dirname` calls above its `_lib/browse-client.ts`.
2. The active perfectworld skills install at `$PERFECTWORLD_SOURCE/`. Read
...
function resolveSdkPath(): string {
  const candidates = [
    path.join(os.homedir(), '.claude', 'skills', 'perfectworld', 'browse', 'src', 'browse-client.ts'),
...
```ts
import { stageSkill } from '<perfectworld-install>perfectworld-browse/src/browser-skill-write';
...
2. If still failing after two retries, OR the failure is an
   ...
   ```ts
   import { discardStaged } from '<perfectworld-install>perfectworld-browse/src/browser-skill-write';
...
Project/branch/task: codified perfectworld-scrape "<intent>" — tests pass against fixture.
ELI10: The script ran clean against the snapshot we captured. Saying yes
moves the staged folder into ~/.perfectworld/browser-skills/ where perfectworld-scrape
...
```ts
import { commitSkill } from '<perfectworld-install>perfectworld-browse/src/browser-skill-write';
...
perfectworld-scrape calls matching '<canonical-trigger>' will run in ~200ms."
...
- **Bun runtime required.** The codified skill runs as a Bun process
  ...
  For now: the skill works on any machine that has perfectworld installed,
...
- Codify match-path perfectworld-scrape results (matched skills are already codified)
- Codify mutating flows (those are perfectworld-automate's job — Phase 2 P0)
- Run skills (that's `$B skill run` — codified skills are run via perfectworld-scrape's
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-log '{"skill":"skillify","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
