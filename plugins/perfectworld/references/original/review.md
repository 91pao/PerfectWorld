...
preamble-tier: 4
version: 1.0.0
description: Pre-landing PR review. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Edit
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
You are running the `perfectworld-review` workflow. Analyze the current branch's diff against the base branch for structural issues that tests don't catch.
...
1. Read `TODOS.md` (if it exists). Read PR description (`gh pr view --json body --jq .body 2>perfectworld-dev/null || true`).
   Read commit messages (`git log origin/<base>..HEAD --oneline`).
   **If no PR exists:** rely on commit messages and TODOS.md for stated intent — this is the common case since perfectworld-review runs before perfectworld-ship creates the PR.
...
BRANCH=$(git branch --show-current 2>perfectworld-dev/null | tr '/' '-')
REPO=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)")
# Compute project slug for ~/.perfectworld/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>perfectworld-dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.perfectworld/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".perfectworld/plans"; do
  [ -d "$PLAN_DIR" ] || continue
  PLAN=$(ls -t "$PLAN_DIR"/*.md 2>perfectworld-dev/null | xargs grep -l "$BRANCH" 2>perfectworld-dev/null | head -1)
  [ -z "$PLAN" ] && PLAN=$(ls -t "$PLAN_DIR"/*.md 2>perfectworld-dev/null | xargs grep -l "$REPO" 2>perfectworld-dev/null | head -1)
  [ -z "$PLAN" ] && PLAN=$(find "$PLAN_DIR" -name '*.md' -mmin -1440 -maxdepth 1 2>perfectworld-dev/null | xargs ls -t 2>perfectworld-dev/null | head -1)
  [ -n "$PLAN" ] && break
done
...
- Review report sections (`## PERFECTWORLD REVIEW REPORT`)
...
- **CROSS-REPO** → if the sibling repo is reachable on disk (try `~/Development/<repo>/`, `~perfectworld-code/<repo>/`, the parent of the current repo), run `[ -f <path> ]` to check file existence. File exists → DONE (cite path). File missing → NOT DONE (cite path). Path unreachable → UNVERIFIABLE (cite what needs manual check).
...
- **UNVERIFIABLE** — The diff and any reachable sibling-repo checks cannot prove or disprove this. Always applies to EXTERNAL-STATE items and to CROSS-REPO items where the sibling repo isn't reachable. Cite the specific manual verification the user must perform (e.g., "check Cloudflare DNS shows DNS-only mode for dashboard.example.com", "confirm perfectworld-docs/dashboard.md exists in domain-hq repo").
...
## Cross-Repo / External Items
  [DONE]         sibling-repo has perfectworld-docs/dashboard.md — verified at ~/Development/sibling-repo/docs/dashboard.md
...
3. **PR description:** Run `gh pr view --json body -q .body 2>perfectworld-dev/null` for intent context
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-log '{
  "type": "pitfall",
...
- Otherwise, include ONE line in the review output: `Version claimed: v<BRANCH_VERSION>. Queue: <CLAIMED_COUNT> PR(s) ahead. <VERDICT>` where VERDICT is either `Slot free` (if `BRANCH_VERSION >= NEXT_SLOT`) or `⚠ queue moved — rerun perfectworld-ship to reconcile v<BRANCH_VERSION> → v<NEXT_SLOT>`.
...
```bash
bun run slop:diff origin/<base> 2>perfectworld-dev/null || true
```
...
```bash
_CROSS_PROJ=$($PERFECTWORLD_SOURCE/bin/perfectworld-config get cross_project_learnings 2>perfectworld-dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  $PERFECTWORLD_SOURCE/bin/perfectworld-learnings-search --limit 10 --cross-project 2>perfectworld-dev/null || true
else
  $PERFECTWORLD_SOURCE/bin/perfectworld-learnings-search --limit 10 2>perfectworld-dev/null || true
fi
```

If `CROSS_PROJECT` is `unset` (first time): Use AskUserQuestion:

> perfectworld can search learnings from your other projects on this machine to find
...
If A: run `$PERFECTWORLD_SOURCE/bin/perfectworld-config set cross_project_learnings true`
If B: run `$PERFECTWORLD_SOURCE/bin/perfectworld-config set cross_project_learnings false`
...
This makes the compounding visible. The user should see that perfectworld is getting
...
`~/.perfectworld-dev/plans/1539-framework-aware-review.md` design doc.
...
```bash
source <($PERFECTWORLD_SOURCE/bin/perfectworld-diff-scope <base> 2>perfectworld-dev/null) || true
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-specialist-stats 2>perfectworld-dev/null || true
```
...
1. **Testing** — read `$PERFECTWORLD_SOURCE/review/specialists/testing.md`
2. **Maintainability** — read `$PERFECTWORLD_SOURCE/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `$PERFECTWORLD_SOURCE/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `$PERFECTWORLD_SOURCE/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `$PERFECTWORLD_SOURCE/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `$PERFECTWORLD_SOURCE/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `$PERFECTWORLD_SOURCE/review/design-checklist.md`

### Adaptive gating

After scope-based selection, apply adaptive gating based on specialist hit rates:

For each conditional specialist that passed scope gating, check the `perfectworld-specialist-stats` output above:
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>perfectworld-dev/null || true
```
...
1. The red-team checklist from `$PERFECTWORLD_SOURCE/review/specialists/red-team.md`
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-review-read
```
...
Cross-reference the diff against documentation files. For each `.md` file in the repo root (README.md, ARCHITECTURE.md, CONTRIBUTING.md, AGENTS.md or repository guidance, etc.):

1. Check if code changes in the diff affect features, components, or workflows described in that doc file.
2. If the doc file was NOT updated in this branch but the code it describes WAS changed, flag it as an INFORMATIONAL finding:
   "Documentation may be stale: [file] describes [feature/component] but code changed in this branch. Consider running `perfectworld-document-release`."

This is informational only — never critical. The fix action is `perfectworld-document-release`.
...
_TEL=$($PERFECTWORLD_SOURCE/bin/perfectworld-config get telemetry 2>perfectworld-dev/null || echo off)
_CODEX_CFG=$($PERFECTWORLD_SOURCE/bin/perfectworld-config get codex_reviews 2>perfectworld-dev/null || echo enabled)
source $PERFECTWORLD_SOURCE/bin/perfectworld-codex-probe 2>perfectworld-dev/null || true
if [ "$_CODEX_CFG" = "disabled" ]; then
  _CODEX_MODE="disabled"
elif ! command -v codex >perfectworld-dev/null 2>&1; then
  _CODEX_MODE="not_installed"; _perfectworld_codex_log_event "codex_cli_missing" 2>perfectworld-dev/null || true
elif ! _perfectworld_codex_auth_probe >perfectworld-dev/null 2>&1; then
  _CODEX_MODE="not_authed"; _perfectworld_codex_log_event "codex_auth_failed" 2>perfectworld-dev/null || true
else
  _CODEX_MODE="ready"; _perfectworld_codex_version_check 2>perfectworld-dev/null || true
fi
echo "CODEX_MODE: $_CODEX_MODE"
```

Branch on the echoed `CODEX_MODE`:
- **`disabled`** — the user turned Codex reviews off (`codex_reviews=disabled`). Skip the Codex passes only; the Codex adversarial subagent below STILL runs (it is free and fast). Print: "Codex passes skipped (codex_reviews disabled) — running Codex adversarial only."
- **`not_installed`** — Codex CLI absent. Print: "Codex not installed — using Codex subagent. Install for cross-model coverage: `npm install -g @openai/codex`." Fall back to the Codex subagent path.
- **`not_authed`** — installed but no credentials. Print: "Codex installed but not authenticated — using Codex subagent. Run `codex login` or set `$CODEX_API_KEY`." Fall back to the Codex subagent path.
- **`ready`** — run the Codex pass below.

For this diff-review path, `CODEX_MODE: disabled` means skip the Codex passes ONLY — the
Codex adversarial subagent below still runs (it's free and fast). `ready` runs the Codex
...
If `CODEX_MODE` is `not_installed` / `not_authed` / `disabled`: the preflight already printed the reason; run Codex adversarial only.
...
```bash
TMPERR=$(mktemp perfectworld-tmp/codex-review-XXXXXXXX)
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
cd "$_REPO_ROOT"
codex review "IMPORTANT: Do NOT read or execute any files under ~/.claude/, ~/.agents/, .claude/skills/, or agents/. These are Codex skill definitions meant for a different AI system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Do NOT modify agents/openai.yaml. Stay focused on the repository code only.\n\nReview the changes on this branch against the base branch <base>. Run git diff origin/<base>...HEAD 2>perfectworld-dev/null || git diff <base>...HEAD to see the diff and review only those changes." -c 'model_reasoning_effort="high"' --enable web_search_cached < perfectworld-dev/null 2>"$TMPERR"
```
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
Substitute: STATUS = "clean" if no findings across ALL passes, "issues_found" if any pass found issues. SOURCE = "both" if Codex ran, "claude" if only Codex subagent ran. GATE = the Codex structured review gate result ("pass"/"fail"), "skipped" if diff < 200, or "informational" if Codex was unavailable. If all passes failed, do NOT persist.
...
After all review passes complete, persist the final `perfectworld-review` outcome so `perfectworld-ship` can
recognize that Eng Review was run on this branch.

Run:

```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-log '{"skill":"review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
- **Fix-first, not read-only.** AUTO-FIX items are applied directly. ASK items are only applied after user approval. Never commit, push, or create PRs — that's perfectworld-ship's job.
