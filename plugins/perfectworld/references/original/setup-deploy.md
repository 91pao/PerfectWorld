...
preamble-tier: 2
version: 1.0.0
description: Configure deployment settings for perfectworld-land-and-deploy.
triggers:
  - configure deploy
  - setup deployment
  - set deploy platform
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"setup-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"setup-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"setup-deploy","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-setup-deploy — Configure Deployment for perfectworld

You are helping the user configure their deployment so `perfectworld-land-and-deploy` works
automatically. Your job is to detect the deploy platform, production URL, health
checks, and deploy status commands — then persist everything to AGENTS.md or repository guidance.

After this runs once, `perfectworld-land-and-deploy` reads AGENTS.md or repository guidance and skips detection entirely.

## User-invocable
When the user types `perfectworld-setup-deploy`, run this skill.
...
```bash
grep -A 20 "## Deploy Configuration" AGENTS.md or repository guidance 2>perfectworld-dev/null || echo "NO_CONFIG"
```

If configuration already exists, show it and ask:

- **Context:** Deploy configuration already exists in AGENTS.md or repository guidance.
...
for f in $(find .github/workflows -maxdepth 1 \( -name '*.yml' -o -name '*.yaml' \) 2>perfectworld-dev/null); do
  [ -f "$f" ] && grep -qiE "deploy|release|production|staging|cd" "$f" 2>perfectworld-dev/null && echo "DEPLOY_WORKFLOW:$f"
done

# Project type
[ -f package.json ] && grep -q '"bin"' package.json 2>perfectworld-dev/null && echo "PROJECT_TYPE:cli"
find . -maxdepth 1 -name '*.gemspec' 2>perfectworld-dev/null | grep -q . && echo "PROJECT_TYPE:library"
```
...
2. Check if `fly` CLI is installed: `which fly 2>perfectworld-dev/null`
3. If installed, verify: `fly status --app {app} 2>perfectworld-dev/null`
4. Infer URL: `https://{app}.fly.dev`
5. Set deploy status command: `fly status --app {app}`
6. Set health check: `https://{app}.fly.dev` (or `perfectworld-health` if the app has one)
...
merge to main, Render picks it up automatically. The "deploy wait" in perfectworld-land-and-deploy
...
1. Check for `vercel` CLI: `which vercel 2>perfectworld-dev/null`
2. If installed: `vercel ls --prod 2>perfectworld-dev/null | head -3`
...
3. **How can perfectworld check if a deploy succeeded?**
   - A) HTTP health check at a specific URL (e.g., perfectworld-health, perfectworld-api/status)
...
Read AGENTS.md or repository guidance (or create it). Find and replace the `## Deploy Configuration` section
if it exists, or append it at the end.

```markdown
## Deploy Configuration (configured by perfectworld-setup-deploy)
- Platform: {platform}
- Production URL: {url}
...
```bash
curl -sf "{health-check-url}" -o perfectworld-dev/null -w "%{http_code}" 2>perfectworld-dev/null || echo "UNREACHABLE"
```

2. If a deploy status command was configured, try it:
```bash
{deploy-status-command} 2>perfectworld-dev/null | head -5 || echo "COMMAND_FAILED"
```
...
Saved to AGENTS.md or repository guidance. perfectworld-land-and-deploy will use these settings automatically.

Next steps:
- Run perfectworld-land-and-deploy to merge and deploy your current PR
- Edit the "## Deploy Configuration" section in AGENTS.md or repository guidance to change settings
- Run perfectworld-setup-deploy again to reconfigure
```
...
- **AGENTS.md or repository guidance is the source of truth.** All configuration lives there — not in a separate config file.
- **Idempotent.** Running perfectworld-setup-deploy multiple times overwrites the previous config cleanly.
