...
preamble-tier: 2
version: 1.0.0
description: Generate missing documentation from scratch for a feature, module, or entire project. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"document-generate","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"document-generate","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
You are running the `perfectworld-document-generate` workflow. Your job: produce **high-quality,
...
2. **From perfectworld-document-release** — the coverage map identified gaps; you fill them
...
1. Determine what to document:
   ...
   - **If called from perfectworld-document-release with gaps**: scope is the specific entities from the coverage map
...
```bash
find . -type f -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.perfectworld/*" -not -path "./dist/*" -not -path "./build/*" -not -path "./.next/*" | head -200
```

2. **Read the entry points.** Identify and read:
   - README.md, ARCHITECTURE.md, CONTRIBUTING.md, AGENTS.md or repository guidance / AGENTS.md
...
2. **Update entry-point files.** Add references to new docs in:
   ...
   - AGENTS.md or repository guidance / AGENTS.md — add to project structure if relevant
...
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
  $PERFECTWORLD_SOURCE/bin/perfectworld-redact --repo-visibility "${REDACT_VIS:-unknown}" --json
