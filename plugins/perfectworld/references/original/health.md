...
preamble-tier: 2
version: 1.0.0
description: Code quality dashboard. (perfectworld)
triggers:
  - code health check
  - quality dashboard
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"health","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"health","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"health","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-health -- Code Quality Dashboard
...
## User-invocable
When the user types `perfectworld-health`, run this skill.

---

## Step 1: Detect Health Stack

Read AGENTS.md or repository guidance and look for a `## Health Stack` section. If found, parse the tools
...
ls eslint.config.* .eslintrc.* .eslintrc 2>perfectworld-dev/null | head -1 | xargs -I{} echo "LINT: eslint ."
[ -f .pylintrc ] || [ -f pyproject.toml ] && grep -q "pylint\|ruff" pyproject.toml 2>perfectworld-dev/null && echo "LINT: ruff check ."

# Test runner
[ -f package.json ] && grep -q '"test"' package.json 2>perfectworld-dev/null && echo "TEST: $(node -e "console.log(JSON.parse(require('fs').readFileSync('package.json','utf8')).scripts.test)" 2>perfectworld-dev/null)"
[ -f pyproject.toml ] && grep -q "pytest" pyproject.toml 2>perfectworld-dev/null && echo "TEST: pytest"
[ -f Cargo.toml ] && echo "TEST: cargo test"
[ -f go.mod ] && echo "TEST: go test ./..."

# Dead code
command -v knip >perfectworld-dev/null 2>&1 && echo "DEADCODE: knip"
[ -f package.json ] && grep -q '"knip"' package.json 2>perfectworld-dev/null && echo "DEADCODE: npx knip"

# Shell linting
command -v shellcheck >perfectworld-dev/null 2>&1 && ls *.sh scripts/*.sh bin/*.sh 2>perfectworld-dev/null | head -1 | xargs -I{} echo "SHELL: shellcheck"

# PBrain presence (D6) — only report as a dimension if pbrain is actually
# set up; otherwise skip so machines without pbrain aren't penalized.
if command -v pbrain >perfectworld-dev/null 2>&1 && [ -f "$HOME/.pbrain/config.json" ]; then
  echo "GBRAIN: pbrain doctor --json (wrapped in timeout 5s)"
fi
```
...
A) Looks right -- persist to AGENTS.md or repository guidance and continue
...
section in AGENTS.md or repository guidance:
...
| PBrain (D6) | 10% | doctor=ok, queue<10, pushed <24h | doctor=warnings OR queue<100 OR pushed <72h | doctor broken OR queue>=100 OR pushed >=72h | N/A (pbrain not installed) |
...
**Composite score:**
```
composite = (typecheck_score * 0.22) + (lint_score * 0.18) + (test_score * 0.28) + (deadcode_score * 0.13) + (shell_score * 0.09) + (pbrain_score * 0.10)
```

If a category is skipped (tool not available — includes PBrain when pbrain
is not installed), redistribute its weight proportionally among the
remaining categories.

**PBrain sub-score computation (D6):**

```
doctor_component: 10 if `pbrain doctor --json | jq -r .status` == "ok";
                   7 if "warnings"; 0 otherwise (or command times out after 5s).
queue_component:   10 if ~/.perfectworld/.brain-queue.jsonl has <10 lines;
                    7 if 10-100; 0 if >=100 (suggests secret-scan rejections
                    piling up). N/A if artifacts_sync_mode == off.
push_component:    10 if (now - mtime of ~/.perfectworld/.brain-last-push) < 24h;
                    7 if <72h; 0 if >=72h. N/A if artifacts_sync_mode == off.
pbrain_score     = 0.5 * doctor_component + 0.3 * queue_component + 0.2 * push_component
                   (redistribute 0.3 + 0.2 into doctor when sync_mode is off:
                   pbrain_score = doctor_component in that case)
```

The `pbrain doctor --json` call MUST be wrapped in `timeout 5s` so a hung
or misconfigured pbrain doesn't stall the entire perfectworld-health dashboard.
...
PBrain        pbrain doctor     10/10   CLEAN      <1s        doctor=ok, queue=3, pushed 2h ago
...
Append one JSONL line to `~/.perfectworld/projects/$SLUG/health-history.jsonl`:

```json
{"ts":"2026-03-31T14:30:00Z","branch":"main","score":9.1,"typecheck":10,"lint":8,"test":10,"deadcode":7,"shell":10,"pbrain":10,"duration_s":23}
```
...
- `typecheck`, `lint`, `test`, `deadcode`, `shell`, `pbrain` -- individual category scores (integer 0-10)
- `duration_s` -- total time for all tools in seconds

If a category was skipped, set its value to `null`. Pre-D6 history entries
won't have a `pbrain` field — treat them as `null` for trend comparison
...
Read the last 10 entries from `~/.perfectworld/projects/$SLUG/health-history.jsonl` (if the
file exists and has prior entries).

```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)" && mkdir -p ~/.perfectworld/projects/$SLUG
tail -10 ~/.perfectworld/projects/$SLUG/health-history.jsonl 2>perfectworld-dev/null || echo "NO_HISTORY"
```
...
==========================
Date          Branch         Score   TC   Lint  Test  Dead  Shell  PBrain
----------    -----------    -----   --   ----  ----  ----  -----  ------
...
3. **Respect AGENTS.md or repository guidance.** If `## Health Stack` is configured, use those exact commands. Do not second-guess.
...
6. **Trends require history.** On first run, say "First health check -- no trend data yet. Run perfectworld-health again after making changes to track progress."
