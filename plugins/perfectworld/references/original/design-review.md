...
preamble-tier: 4
version: 2.0.0
description: "Designer's eye QA: finds visual inconsistency, spacing issues, hierarchy problems, AI slop patterns, and slow interactions — then fixes them. (perfectworld)"
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-design-review: Design Audit → Fix → Verify
...
|-----------|---------|-----------------:|
| Target URL | (auto-detect or ask) | `https:/perfectworld-myapp.com`, `http:/perfectworld-localhost:3000` |
...
```bash
$B status 2>perfectworld-dev/null | grep -q "Mode: cdp" && echo "CDP_MODE=true" || echo "CDP_MODE=false"
```
...
"Your working tree has uncommitted changes. perfectworld-design-review needs a clean tree so each design fix gets its own atomic commit."
...
[ -f Gemfile ] && grep -q "rails" Gemfile 2>perfectworld-dev/null && echo "FRAMEWORK:rails"
[ -f package.json ] && grep -q '"next"' package.json 2>perfectworld-dev/null && echo "FRAMEWORK:nextjs"
# Check for existing test infrastructure
ls jest.config.* vitest.config.* playwright.config.* .rspec pytest.ini pyproject.toml phpunit.xml 2>perfectworld-dev/null
ls -d test/ tests/ spec/ __tests__/ cypress/ e2e/ 2>perfectworld-dev/null
# Check opt-out marker
[ -f .perfectworld/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.perfectworld/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.perfectworld/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.perfectworld/no-test-bootstrap` and re-run." Continue without tests.
...
```bash
# Check CI provider
ls -d .github/ 2>perfectworld-dev/null && echo "CI:github"
ls .gitlab-ci.yml .circleci/ bitrise.yml 2>perfectworld-dev/null
```
...
### B7. Update AGENTS.md or repository guidance

First check: If AGENTS.md or repository guidance already has a `## Testing` section → skip. Don't duplicate.
...
Only commit if there are changes. Stage all bootstrap files (config, test directory, TESTING.md, AGENTS.md or repository guidance, .github/workflows/test.yml if created):
`git commit -m "chore: bootstrap test framework ({framework name})"`

---

**Find the perfectworld designer (optional — enables target mockup generation):**
...
_ROOT=$(git rev-parse --show-toplevel 2>perfectworld-dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.agents/skills/perfectworld/design/dist/design" ] && D="$_ROOT/.agents/skills/perfectworld/design/dist/design"
[ -z "$D" ] && D="$HOME/.agents/skills/perfectworld/design/dist/design"
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
Commands:
- `$D generate --brief "..." --output perfectworld-path.png` — generate a single mockup
- `$D variants --brief "..." --count 3 --output-dir perfectworld-path/` — generate N style variants
- `$D compare --images "a.png,b.png,c.png" --output perfectworld-path/board.html --serve` — comparison board + HTTP server
- `$D serve --html perfectworld-path/board.html` — serve comparison board and collect feedback via HTTP
- `$D check --image perfectworld-path.png --brief "..."` — vision quality gate
- `$D iterate --session perfectworld-path/session.json --feedback "..." --output perfectworld-path.png` — iterate

**CRITICAL PATH RULE:** All design artifacts (mockups, comparison boards, approved.json)
MUST be saved to `~/.perfectworld/projects/$SLUG/designs/`, NEVER to `.context/`,
`docs/designs/`, `perfectworld-tmp/`, or any project-local directory. Design artifacts are USER
...
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
REPORT_DIR="$HOME/.perfectworld/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)"
...
$B url
```
If URL contains `perfectworld-login`, `perfectworld-signin`, `perfectworld-auth`, or `perfectworld-sso`: the site requires authentication. AskUserQuestion: "This site requires authentication. Want to import cookies from your browser? Run `perfectworld-setup-browser-cookies` first if needed."
...
**Local:** `.perfectworld/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)" && mkdir -p ~/.perfectworld/projects/$SLUG
```
Write to: `~/.perfectworld/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https:/perfectworld-developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + perfectworld design methodology.
...
```
~/.perfectworld/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
```bash
command -v codex >perfectworld-dev/null 2>&1 && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
```
...
```bash
TMPERR_DESIGN=$(mktemp perfectworld-tmp/codex-design-XXXXXXXX)
...
Be specific. Reference file:line for every finding." -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' --enable web_search_cached < perfectworld-dev/null 2>"$TMPERR_DESIGN"
```
...
Use the same scorecard format as perfectworld-plan-design-review (shown above). Fill in from both outputs.
Merge findings into the triage with `[codex]` / `[subagent]` / `[cross-model]` tags.

**Log the result:**
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
If the perfectworld designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
For CSS-only fixes: skip entirely. CSS regressions are caught by re-running perfectworld-design-review.

If the fix involved JS behavior: follow the same procedure as perfectworld-qa Phase 8e.5 (study existing
...
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)" && mkdir -p ~/.perfectworld/projects/$SLUG
```
Write a one-line summary to `~/.perfectworld/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
2. **Fixed findings that were in TODOS.md** → annotate with "Fixed by perfectworld-design-review on {branch}, {date}"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-log '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
