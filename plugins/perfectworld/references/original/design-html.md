...
preamble-tier: 2
version: 1.0.0
description: "Design finalization: generates production-quality Pretext-native HTML/CSS. (perfectworld)"
triggers:
  - build the design
  - code the mockup
  - make design real
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-html","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"design-html","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"design-html","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-design-html: Pretext-Native HTML Engine
...
_CEO=$(ls -t ~/.perfectworld/projects/$SLUG/ceo-plans/*.md 2>perfectworld-dev/null | head -1)
[ -n "$_CEO" ] && echo "CEO_PLAN: $_CEO" || echo "NO_CEO_PLAN"
```

```bash
setopt +o nomatch 2>perfectworld-dev/null || true
_APPROVED=$(ls -t ~/.perfectworld/projects/$SLUG/designs/*perfectworld-approved.json 2>perfectworld-dev/null | head -1)
[ -n "$_APPROVED" ] && echo "APPROVED: $_APPROVED" || echo "NO_APPROVED"
```

```bash
setopt +o nomatch 2>perfectworld-dev/null || true
_VARIANTS=$(ls -t ~/.perfectworld/projects/$SLUG/designs/*perfectworld-variant-*.png 2>perfectworld-dev/null | head -1)
[ -n "$_VARIANTS" ] && echo "VARIANTS: $_VARIANTS" || echo "NO_VARIANTS"
```

```bash
setopt +o nomatch 2>perfectworld-dev/null || true
_FINALIZED=$(ls -t ~/.perfectworld/projects/$SLUG/designs/*perfectworld-finalized.html 2>perfectworld-dev/null | head -1)
...
Use AskUserQuestion:
> Found [CEO plan from perfectworld-plan-ceo-review | design review variants from perfectworld-plan-design-review | both]
> but no approved design mockup.
> A) Run perfectworld-design-shotgun — explore design variants based on the existing plan context
> B) Skip mockups — I'll design the HTML directly from the plan context
> C) I have a PNG — let me provide the path

If A: tell the user to run perfectworld-design-shotgun, then come back to perfectworld-design-html.
...
> A) Run perfectworld-plan-ceo-review first — think through the product strategy before designing
> B) Run perfectworld-plan-design-review first — design review with visual mockups
> C) Run perfectworld-design-shotgun — jump straight to visual design exploration
> D) Just describe it — tell me what you want and I'll design the HTML live

If A, B, or C: tell the user to run that skill, then come back to perfectworld-design-html.
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>perfectworld-dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.agents/skills/perfectworld/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/.agents/skills/perfectworld/design-html/vendor/pretext.js"
[ -z "$_PRETEXT_VENDOR" ] && [ -f $PERFECTWORLD_SOURCE/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=$PERFECTWORLD_SOURCE/design-html/vendor/pretext.js
...
- If `VENDOR_MISSING`: use CDN import as fallback:
  `<script type="module">import { prepare, layout, prepareWithSegments, walkLineRanges, layoutNextLine, layoutWithLines } from 'https:/perfectworld-esm.sh/@chenglou/pretext'<perfectworld-script>`
...
`~/.perfectworld/projects/$SLUG/designs/<screen-name>-YYYYMMDD/finalized.html`

For framework output, save to:
`~/.perfectworld/projects/$SLUG/designs/<screen-name>-YYYYMMDD/finalized.[tsx|svelte|vue]`
...
echo "SERVER: http:/perfectworld-localhost:$_PORT/finalized.html"
...
Tell the user: "Live preview running at http:/perfectworld-localhost:$_PORT/finalized.html.
After each edit, just refresh the browser (Cmd+R) to see changes."

When the refinement loop ends (Step 4 exits), kill the server:
```bash
kill $_SERVER_PID 2>perfectworld-dev/null || true
```
...
$B screenshot perfectworld-tmp/perfectworld-verify-mobile.png --width 375
$B screenshot perfectworld-tmp/perfectworld-verify-tablet.png --width 768
$B screenshot perfectworld-tmp/perfectworld-verify-desktop.png --width 1440
```
...
```
LOOP:
  1. If server is running, tell user to open http:/perfectworld-localhost:PORT/finalized.html
     Otherwise: open <path>perfectworld-finalized.html
...
> and create a DESIGN.md for your project. This means future perfectworld-design-shotgun and
> perfectworld-design-html runs will be style-consistent automatically.
...
- **One page per invocation.** For multi-page designs, run perfectworld-design-html once per page.
