...
preamble-tier: 2
version: 1.0.0
description: "Self-tuning question sensitivity + developer psychographic for perfectworld (v1: observational). (perfectworld)"
triggers:
  - tune questions
  - stop asking me that
  - too many questions
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-tune","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"plan-tune","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"plan-tune","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-plan-tune — Question Tuning + Developer Profile (v1 observational)
...
1. **Consent gate.** If `question_tuning` is `false` AND
   `~/.perfectworld/.question-tuning-prompted` is missing → run `Consent + opt-in`
   below. Honor the answer with a marker write either way; do not re-prompt.
2. **Setup gate.** If `question_tuning` is `true` AND
   `~/.perfectworld/developer-profile.json`'s `declared` object is empty AND
   `~/.perfectworld/.declared-setup-prompted` is missing → run `5-Q setup` below.
   Touch the marker after setup completes OR is declined.
3. **Dream-cycle gate (Layer 8 / cathedral T10/T11).** If
   `~/.perfectworld/projects/<slug>perfectworld-distillation-proposals.json` exists AND has
...
9. **"Dream cycle" / "distill" / "what have I been free-texting"** →
   run `Dream cycle distill` below (triggers `perfectworld-distill-free-text`).
10. **"Turn it off" / "disable"** → `$PERFECTWORLD_SOURCE/bin/perfectworld-config set question_tuning false`
11. **"Turn it on" / "enable"** → `$PERFECTWORLD_SOURCE/bin/perfectworld-config set question_tuning true && touch ~/.perfectworld/.question-tuning-prompted`
...
`~/.perfectworld/.question-tuning-prompted` is missing. The user has never been
asked.

**Privacy note.** perfectworld defaults `question_tuning` to `false` for every user.
...
rationale). If the user is a contributor (`perfectworld_contributor: true`), the
...
1. Detect contributor state (for prompt framing only, not for auto-action):
   ...
   _CONTRIB=$($PERFECTWORLD_SOURCE/bin/perfectworld-config get perfectworld_contributor 2>perfectworld-dev/null || echo "false")
...
2. AskUserQuestion (use the contributor-specific framing only if `_CONTRIB=true`,
   ...
   **General framing:**
   > Question tuning is off. perfectworld can learn which of its prompts you find
   > valuable vs noisy — so over time, perfectworld stops asking questions you've
   > already answered the same way. It takes about 2 minutes to set up your
   > initial profile. v1 is observational: perfectworld tracks your preferences
   > and shows you a profile, but doesn't silently change skill behavior yet.
   > Logs stay local (`~/.perfectworld/projects/<slug>perfectworld-question-log.jsonl`).
   >
   ...
   **Contributor framing (only if `_CONTRIB=true`):**
   > You're a perfectworld contributor. Question tuning isn't on by default for
   ...
   > `~/.perfectworld/projects/<slug>perfectworld-question-log.jsonl` — nothing leaves your
...
3. ALWAYS touch the marker, regardless of choice:
   ```bash
   touch ~/.perfectworld/.question-tuning-prompted
   ```

4. If A or B: enable:
   ```bash
   $PERFECTWORLD_SOURCE/bin/perfectworld-config set question_tuning true
   ```

5. If C: do nothing else. Tell the user: "Question tuning stays off. Re-enable
   any time with `perfectworld-plan-tune enable` or `perfectworld-config set question_tuning true`."
...
- Standalone via Step 0's setup gate: `question_tuning` is already `true`
  (user opted in via perfectworld-config or earlier `perfectworld-plan-tune enable`) AND
  `declared` is empty AND `~/.perfectworld/.declared-setup-prompted` is missing.
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   `~/.perfectworld/developer-profile.json` under `declared.{dimension}`:

   ```bash
   # Ensure profile exists
   $PERFECTWORLD_SOURCE/bin/perfectworld-developer-profile --read >perfectworld-dev/null
   # Update declared dimensions atomically
   eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-paths)"
   _PROFILE="$PERFECTWORLD_STATE_ROOT/developer-profile.json"
   bun -e "
     const fs = require('fs');
...
2. Touch the marker so the Setup gate doesn't re-fire:
   ```bash
   touch ~/.perfectworld/.declared-setup-prompted
   ```
   ...
   anytime with `perfectworld-plan-tune setup` (Step 0 power-user shortcut).

3. Tell the user: "Profile set. Question tuning is on. Use `perfectworld-plan-tune`
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-developer-profile --profile
```
...
- Show the vibe (archetype) from `perfectworld-developer-profile --vibe` — the
...
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-paths)"
_LOG="$PERFECTWORLD_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ ! -f "$_LOG" ]; then
  echo "NO_LOG"
else
  bun -e "
...
If `NO_LOG`, tell the user: "No questions logged yet. As you use perfectworld skills,
perfectworld will log them here."
...
The user has asked to change a preference, either via the `perfectworld-plan-tune` menu
...
4. Write:
   ```bash
   $PERFECTWORLD_SOURCE/bin/perfectworld-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
6. If the user was responding to an inline `tune:` during another skill, note
   ...
   `perfectworld-plan-tune` invocations, `source: "plan-tune"` is correct.
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-developer-profile --gap
```
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-preference --stats
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-paths)"
_LOG="$PERFECTWORLD_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ -f "$_LOG" ]; then
...
$PERFECTWORLD_SOURCE/bin/perfectworld-developer-profile --profile | bun -e "
...
"
echo '---DISTILL---'
$PERFECTWORLD_SOURCE/bin/perfectworld-distill-free-text --status
```
...
Run `perfectworld-question-preference --write '{"question_id":"<id>","preference":
...
hook captured but cannot enforce against (no `<perfectworld-qid:foo>` marker in
...
invokes via `perfectworld-plan-tune distill` / `dream`.

**Flow:**

1. Show the proposals:
   ```bash
   $PERFECTWORLD_SOURCE/bin/perfectworld-distill-apply --list
   ```
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   nugget to pbrain when configured.

   For `memory-nugget`:
   ```bash
   # If pbrain is configured, mirror via MCP first.
   # (Pseudo — actual pbrain call happens at the agent layer via
   # mcp__pbrain__put_page; the bin records the published flag.)
   $PERFECTWORLD_SOURCE/bin/perfectworld-distill-apply --proposal N --pbrain-published true|false
   ```

   For `preference`:
   ```bash
   $PERFECTWORLD_SOURCE/bin/perfectworld-distill-apply --proposal N
   ```
...
4. **On decline**: skip without marking. User can re-decide later (the
   ...
   `perfectworld-distill-apply --proposal N --dismiss` (not implemented in T11;
   for now, regenerate via next distill run with corrected free-text).

5. **pbrain integration.** When `mcp__pbrain__*` tools are available in
   this session:
   - On `memory-nugget` apply: `mcp__pbrain__put_page` with the nugget +
     `mcp__pbrain__extract_facts` + `mcp__pbrain__add_tag` per the cathedral
     plan D9 routing. Then pass `--pbrain-published true` to the bin so
     the proposals file records the mirror.
   - When pbrain isn't configured (no MCP tools), the bin's local file
...
**When this fires.** The user invokes `perfectworld-plan-tune distill` / `dream` /
...
1. Run distill:
   ```bash
   $PERFECTWORLD_SOURCE/bin/perfectworld-distill-free-text
   ```

2. If `RATE_CAPPED`: tell the user "You've hit today's 3 distills/day cap.
   Run again tomorrow, or `perfectworld-plan-tune stats` for run history."
3. If `NO_FREE_TEXT`: tell the user "No free-text answers since the last
   distill. Keep using perfectworld — `Other` responses on AskUserQuestion feed
   this loop."
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-distill-free-text --background
```
