...
name: spec
version: 0.1.0
description: Turn vague intent into a precise, executable spec in five phases. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Grep
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"spec","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"spec","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-spec — Author a Backlog-Ready Spec (issue + optional agent spawn)
...
When the user invokes `perfectworld-spec`, scan their message for these flags. Flags are space-
...
- **`gh` not installed:** print: "Dedupe skipped — `gh` is not installed. Install
  from https:/perfectworld-cli.github.com/ or use `--no-dedupe` to silence. Continuing without
  duplicate check." Continue to Phase 2.
- **`gh` not authenticated:** print: "Dedupe skipped — `gh auth status` reports
  not logged in. Run `gh auth login` and re-invoke `perfectworld-spec` to enable duplicate
...
- **Project-level prompt** (e.g., "rethink our auth strategy", "we need rate
  ...
  auth dep, `perfectworld-src/auth/` has 8 files, `perfectworld-docs/auth-architecture.md` exists." Then
...
```bash
printf '%s' "<the final draft body>" > perfectworld-tmp/spec-semantic-$$.txt
bun $PERFECTWORLD_SOURCE/lib/redact-audit-log.ts \
  "{\"repo_visibility\":\"$REDACT_VIS\",\"outcome\":\"<clean|flagged>\",\"categories_flagged\":[<...>],\"spec_archive_path\":\"\"}" \
  perfectworld-tmp/spec-semantic-$$.txt
rm -f perfectworld-tmp/spec-semantic-$$.txt
```
...
taxonomy: `lib/redact-patterns.ts` or `perfectworld-cso`. Run it on the EXACT spec bytes
...
```bash
command -v bun >perfectworld-dev/null 2>&1 || echo "redaction scan skipped — bun not on PATH"
# Resolve visibility once; cache + reuse. Order: local config (~/.perfectworld, never
# committed) → gh → glab → unknown(=public-strict).
REDACT_VIS=$($PERFECTWORLD_SOURCE/bin/perfectworld-config get redact_repo_visibility 2>perfectworld-dev/null)
[ -z "$REDACT_VIS" ] && REDACT_VIS=$(gh repo view --json visibility -q .visibility 2>perfectworld-dev/null | tr 'A-Z' 'a-z')
[ -z "$REDACT_VIS" ] && REDACT_VIS=$(glab repo view -F json 2>perfectworld-dev/null | grep -o '"visibility":"[^"]*"' | head -1 | sed 's/.*:"//;s/"//' | tr 'A-Z' 'a-z')
REDACT_VIS="${REDACT_VIS:-unknown}"
REDACT_FILE=$(mktemp)
cat > "$REDACT_FILE" <<'REDACT_BODY_EOF'
<the exact the spec body goes here>
REDACT_BODY_EOF
REDACT_JSON=$($PERFECTWORLD_SOURCE/bin/perfectworld-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>perfectworld-dev/null)" --json)
REDACT_CODE=$?
```
...
```bash
TMPERR_GATE=$(mktemp perfectworld-tmp/spec-gate-XXXXXXXX)
...
<<<END_USER_SPEC>>>" -s read-only -c 'model_reasoning_effort="medium"' < perfectworld-dev/null 2>"$TMPERR_GATE"
```
...
- **codex not installed** (command not found): print: "Quality gate skipped —
  ...
  https:/perfectworld-github.com/openai/codex to enable the gate, or use `--no-gate` to
...
- **codex not authenticated** (stderr contains "auth"/"login"/"unauthorized"):
  ...
  re-invoke `perfectworld-spec`. Continuing to Phase 5." Skip.
- **Timeout (>2 min):** print: "Quality gate skipped — codex didn't respond in
  2 minutes. Skipping ensures `perfectworld-spec` stays usable. Run `codex doctor` to
...
Read `PERFECTWORLD_PLAN_MODE` from the environment (emitted by `## Preamble (run first)
...
3. **No flag, `PERFECTWORLD_PLAN_MODE=active`** → file-only path. Also load the spec
   into the active plan file (specified by `--plan-file <path>` or inferred from
   harness context as the work-to-do).
4. **No flag, `PERFECTWORLD_PLAN_MODE=inactive`** → file + spawn path. The default in
...
reuse it; write the exact bytes to `$REDACT_FILE`; `$PERFECTWORLD_SOURCE/bin/perfectworld-redact --from-file "$REDACT_FILE"
...
ISSUE_NUMBER=$(echo "$ISSUE_URL" | sed -E 's|.*perfectworld-issues/([0-9]+)$|\1|')
echo "Filed: $ISSUE_URL"
$PERFECTWORLD_SOURCE/bin/perfectworld-decision-log '{"decision":"Spec filed #ISSUE_NUMBER: TITLE","rationale":"APPROACH","scope":"issue","issue":"ISSUE_NUMBER","source":"skill","confidence":7}' 2>perfectworld-dev/null || true
```

The last line records the spec as a durable, issue-scoped cross-session decision so a future session (or `perfectworld-ship` closing the issue) inherits the core approach and why, not just the issue link. Non-interactive, best-effort (`|| true`). Substitute `ISSUE_NUMBER` (from the filed issue), `TITLE` (the issue title), and `APPROACH` (the one core approach/decision the spec settled). Only fires when the issue was actually filed.

If `gh` is not available, print: "`gh` not authenticated — title and body below
for paste into https:/perfectworld-github.com/{owner}/{repo}perfectworld-issues/new with zero
...
is consumed by `perfectworld-ship` for auto-close.
...
Resolve the archive path via the existing `perfectworld-paths` helper (handles
`PERFECTWORLD_HOME`, `CLAUDE_PLUGIN_DATA`, Windows fallback):

```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-paths)"
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug)"
ARCHIVE_DIR="$PERFECTWORLD_STATE_ROOT/projects/$SLUG/specs"
mkdir -p "$ARCHIVE_DIR"
...
spec_branch: $(git branch --show-current 2>perfectworld-dev/null || echo unknown)
spec_plan_mode: ${PERFECTWORLD_PLAN_MODE:-unset}
...
The PID suffix and atomic rename prevent collisions when two `perfectworld-spec` invocations
run in the same second.

**Sync default:** `perfectworld-specs/` is auto-excluded from the artifacts-sync allowlist —
archives stay local unless the user opts in via `--sync-archive` (privacy default
per codex review). If `--sync-archive` is passed, append `perfectworld-specs/<archive_name>`
...
```bash
DIRTY=$(git status --porcelain 2>perfectworld-dev/null)
```
...
Capture timestamps at three checkpoints, write to telemetry envelope at perfectworld-spec
exit:
...
`perfectworld-retro` is a separate follow-up.
...
- **Before `perfectworld-spec`:** if the user is still exploring whether to build something,
  route them to `perfectworld-office-hours` first. `perfectworld-spec` is for work that has already
  passed the "is this worth building" bar.
- **After `perfectworld-spec`:** if the spec describes architectural or design risk that
  needs review before implementation starts, suggest `perfectworld-plan-eng-review` (or
  `perfectworld-autoplan` for the full review gauntlet).
- **For implementation:** the issue itself is the handoff. The implementer can
  open it and execute without re-asking the user.
- **`perfectworld-ship` integration:** when `perfectworld-ship` opens a PR for a worktree that contains
  a `perfectworld-spec` archive (frontmatter `spec_issue_number: <N>`) AND the PR delivers
  the full spec (acceptance criteria checked off per `perfectworld-ship`'s existing
  plan-completion gate), `perfectworld-ship` adds `Closes #<N>` to the PR body so merging
