...
preamble-tier: 2
version: 1.0.0
description: Keep pbrain current with this repo's code and refresh agent search guidance in AGENTS.md or repository guidance. Wraps the perfectworld-pbrain-sync orchestrator with state (perfectworld)
triggers:
  - sync pbrain
  - refresh pbrain
  - reindex repo
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"sync-pbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"sync-pbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"sync-pbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-sync-pbrain — Keep pbrain current and teach the agent to use it

You are running the canonical "keep this brain up to date" verb. perfectworld-setup-pbrain
installs pbrain once; perfectworld-sync-pbrain runs every time the user wants the brain
refreshed against this repo's current state, and refreshes the agent-side
guidance in AGENTS.md or repository guidance so the coding agent knows when to prefer `pbrain`
search over Grep.

**Architecture (post-codex review):** This skill uses pbrain v0.20.0+'s
**native code surfaces** (`pbrain sources add`, `pbrain sync --strategy code`,
`pbrain reindex-code`, `pbrain code-def/code-refs/code-callers/code-callees`).
It does NOT use `pbrain import` (that path is for markdown directories).
It does NOT touch `~/.perfectworld/` indexing (the existing `perfectworld-pbrain-source-wireup`
owns that — never double-store).

## User-invocable

When the user types `perfectworld-sync-pbrain`, run this skill. Argument modes (parsed by
the skill itself, not a dispatcher binary):

- `perfectworld-sync-pbrain` — incremental sync (default; mtime fast-path; ~50ms steady-state)
- `perfectworld-sync-pbrain --full` — full code reindex via `pbrain reindex-code` (~25-35 min on a big repo). Auto-builds the call graph (`pbrain dream`) **only when it was never built**.
- `perfectworld-sync-pbrain --dream` — build this source's call graph (`pbrain code-callers`/`code-callees`) via a source-scoped `pbrain dream --source <id>` cycle; ~minutes; runs lock-free after the sync stages. Always forces, even if already built. Only produces a graph on a code-aware schema pack; otherwise the run reports a WARN explaining why the graph is still empty.
- `perfectworld-sync-pbrain --no-dream` — skip the dream cycle that `--full` would otherwise auto-run.
- `perfectworld-sync-pbrain --code-only` — only run the code stage; skip memory + brain-sync
- `perfectworld-sync-pbrain --dry-run` — preview what would sync; no writes anywhere
- `perfectworld-sync-pbrain --no-memory` / `--no-brain-sync` — selectively skip stages
- `perfectworld-sync-pbrain --quiet` — suppress per-stage output
- `perfectworld-sync-pbrain --refresh-cache` — force-rebuild brain-aware planning cache (v1.48; replaces perfectworld-brain-refresh-context per D1 fold). Skips code + memory stages; routes to `perfectworld-brain-cache refresh --project <slug>`.
- `perfectworld-sync-pbrain --audit` — emit summary of perfectworld-owned pages per project + sensitive-content audit (v1.48 / D10 lifecycle). Read-only.

Pass-through args go straight to the orchestrator at
`$PERFECTWORLD_SOURCE/bin/perfectworld-pbrain-sync.ts`.

**`--refresh-cache` short-circuit:** when this flag is present, the skill
runs ONLY the cache refresh (`perfectworld-brain-cache refresh --project <slug>`
for the current worktree's slug, plus a cross-project refresh of
user-profile if `perfectworld/user-profile/<user-slug>` exists). Code +
memory + brain-sync stages are skipped. Useful when the user knows the
brain has new info perfectworld should pick up before the next planning skill.

**`--audit` short-circuit:** when this flag is present, the skill runs
`perfectworld-brain-cache list --project <slug> --json`, summarizes by page
...
Before doing anything, check that perfectworld-setup-pbrain has been run on this Mac.

```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-pbrain-detect 2>perfectworld-dev/null
```

**Brain trust policy gate (v1.48 / Phase 1.5 / D4 — added by T13+T5c):**
If `pbrain_mcp_mode == "remote-http"` from the detect output AND the per-
...
Step 9.5 wording in `perfectworld-setup-pbrain` (personal vs shared, with persistence
...
per-machine pbrain engine (PGLite or whatever `pbrain config` points to),
with each worktree of a repo registered as its own source. **Memory stage
also runs locally** in local-stdio MCP mode — `perfectworld-memory-ingest` shells
out to `pbrain import` against the same local engine. In remote-http MCP
mode (Path 4), the memory stage instead persists staged markdown to
`~/.perfectworld/transcripts/<run-id>/` and the artifacts pipeline pushes it to
the brain admin's pull job (plan D11). Brain-sync (the `perfectworld-brain-sync`
...
Also check the per-repo trust policy. If `perfectworld-pbrain-repo-policy get` for
this repo returns `deny`, STOP:

> "This repo's pbrain trust policy is `deny`. Run `perfectworld-setup-pbrain --repo` to
...
Read `pbrain_local_status` from the Step 1 detect output. Branch as follows
BEFORE invoking the orchestrator:

- **`ok`**: proceed to Step 2 normally.
- **`no-cli`**: STOP. "Local pbrain CLI not installed. Run `perfectworld-setup-pbrain`
  first."
- **`missing-config`** AND `pbrain_mcp_mode == "remote-http"`: tell the user
  "Your brain queries (the `mcp__pbrain__*` tools) work via remote MCP, but
  symbol code search needs a local PGLite. Run `perfectworld-setup-pbrain` and pick
  'Yes' at the new 'local code index' prompt (Step 4.5), or run
  `pbrain init --pglite --json --embedding-model voyage:voyage-code-3 --embedding-dimensions 1024`
...
- **`missing-config`** AND `pbrain_mcp_mode != "remote-http"`: STOP. "Local
  pbrain CLI is installed but no engine config. Run `perfectworld-setup-pbrain` first."
- **`broken-config`** OR **`broken-db`**: STOP with a clear message:
  ```
  Local pbrain config at ~/.pbrain/config.json points at an unreachable
  engine (status: {pbrain_local_status}). Two options:
    1. Re-run perfectworld-setup-pbrain — Step 1.5 offers Retry / Switch to PGLite /
       Switch brain mode / Quit (plan D4).
    2. Repair manually: mv ~/.pbrain/config.json ~/.pbrain/config.json.bak
       && pbrain init --pglite --json --embedding-model voyage:voyage-code-3 \
          --embedding-dimensions 1024   (drop voyage flags if VOYAGE_API_KEY unset)
  Re-run perfectworld-sync-pbrain after.
  ```
...
```bash
bun run $PERFECTWORLD_SOURCE/bin/perfectworld-pbrain-sync.ts <user-args>
```
...
still run. State is persisted to `~/.perfectworld/.pbrain-sync-state.json` via
tmp-file + atomic rename. Concurrent runs are blocked by a lock file at
`~/.perfectworld/.sync-pbrain.lock` (5-min stale-takeover).

---

## Step 3: Code-index health check

After the sync run, query pbrain for the cwd source's page_count:

```bash
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' ~/.perfectworld/.pbrain-sync-state.json 2>perfectworld-dev/null \
  | head -1 | sed 's/.*"source_id":"//;s/".*//')
PAGES=$(pbrain sources list --json 2>perfectworld-dev/null \
  | jq -r --arg id "$SOURCE_ID" '.sources[] | select(.id==$id) | .page_count' 2>perfectworld-dev/null \
  || echo 0)
...
> D1 — This repo has 0 indexed pages in pbrain. Run a full code reindex now?
>
> ELI10: pbrain hasn't indexed this repo's code yet. The semantic search
> tools (`pbrain search`, `code-def`, `code-refs`) will return nothing
...
> and Step 2 of this skill already verified pbrain is configured correctly.
>
> Note: options differ in kind, not coverage — no completeness score.
>
> A) Run perfectworld-sync-pbrain --full now (recommended)
...
`pbrain code-callers` / `code-callees` (who-calls-this / what-this-calls) return
`count: 0` until a `pbrain dream` cycle runs the `resolve_symbol_edges` phase for
...
that doesn't declare it (e.g. `pbrain-base` / `pbrain-base-v2`), a `dream` cycle
...
(a WARN row) rather than claiming a build that didn't happen. pbrain exposes pack
...
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' ~/.perfectworld/.pbrain-sync-state.json 2>perfectworld-dev/null \
  | head -1 | sed 's/.*"source_id":"//;s/".*//')
CYCLE=$(pbrain doctor --json --fast 2>perfectworld-dev/null \
  | jq -r --arg id "$SOURCE_ID" '
      ...
      | if $c.status=="ok" then "completed"
        ...
        else "unknown" end' 2>perfectworld-dev/null || echo unknown)
...
>
> ELI10: `pbrain code-callers`/`code-callees` (who calls this function / what it
> calls) return nothing until the `resolve_symbol_edges` phase runs for this
> source. `pbrain dream --source <this source>` runs it (scoped to this
...
> Note: options differ in kind, not coverage — no completeness score.
>
> A) Run perfectworld-sync-pbrain --dream now (recommended)
> B) Skip — I'll run it later
...
## Step 4: Refresh `## PBrain Search Guidance` block in AGENTS.md or repository guidance

Capability check (per perfectworld-plan-eng-review §6):

```bash
SLUG="_capability_check_$$"
CAPABILITY_OK=0
if [ -f ~/.pbrain/config.json ] && \
   pbrain --version 2>perfectworld-dev/null | grep -q '^pbrain '; then
  ...
  if echo "ping" | pbrain put "$SLUG" >perfectworld-dev/null 2>&1; then
    ...
    for _attempt in 1 2 3; do
      if pbrain search "ping" 2>perfectworld-dev/null | grep -q "$SLUG"; then
        CAPABILITY_OK=1
        break
      fi
      sleep 1
...
pbrain delete "$SLUG" 2>perfectworld-dev/null || true
```

Then update AGENTS.md or repository guidance based on capability state:
...
end of AGENTS.md or repository guidance if it doesn't. NEVER duplicate. Block is machine-AGNOSTIC
(no engine, no page counts, no last-sync time — those are in the existing
`## PBrain Configuration` block).
...
`.pbrain-source` file in the repo root (kubectl-style context).
`pbrain code-def`, `code-refs`, `code-callers`, `code-callees`, `search`, and
`query` from anywhere under this worktree route to that source by default —
no `--source` flag needed (pbrain >= 0.41.38.0; on older pbrain the call-graph
commands need `--source "$(cat .pbrain-source)"`). Conductor sibling worktrees
of the same repo each have their own pin and their own indexed pages, so
semantic results match the code on disk here.

Call-graph queries (`code-callers`/`code-callees`) also need the graph to be
built first — run `perfectworld-sync-pbrain --dream` (or `--full`) if they return
`count: 0`. This only works if this source's pbrain schema pack extracts code
symbols; on a non-code-aware pack `--dream` completes but the graph stays empty
and reports a WARN. `code-def`/`code-refs` need the same extraction.

Two indexed corpora available via the `pbrain` CLI:
- This worktree's code (auto-pinned via `.pbrain-source`).
...
file globs. Run `perfectworld-sync-pbrain` after meaningful code changes; for ongoing
auto-sync across all worktrees, run `pbrain autopilot --install` once per
machine — pbrain's daemon handles incremental refresh on a schedule.

Safety: don't run `perfectworld-sync-pbrain` while `pbrain autopilot` is active — the
orchestrator refuses destructive source ops when it detects a running autopilot
to avoid racing it (#1734). Prefer registering user repos with `pbrain sources
...
from `<!-- perfectworld-pbrain-search-guidance:start -->` through
`<!-- perfectworld-pbrain-search-guidance:end -->`. If those markers are missing,
search for `## PBrain Search Guidance (configured by perfectworld-sync-pbrain)` heading
and replace from there to the next `## ` or EOF. If no heading exists, append
the entire block at the end of AGENTS.md or repository guidance.

**Atomic write:** write the new AGENTS.md or repository guidance content to a tmp file alongside it
(e.g., `AGENTS.md or repository guidance.sync-pbrain.tmp`) then `mv` to atomic-rename, so a crash
mid-write never leaves the file half-modified.

**If `CAPABILITY_OK=0`** — REMOVE the block entirely if present. Use the same
Edit tool to strip the start/end-marker region. The `## PBrain Configuration`
block stays in place (it's a record of the install, not a capability claim).

Do NOT crash if AGENTS.md or repository guidance is missing or unwritable — log a warning and
continue.
...
Print a status block matching `perfectworld-setup-pbrain` Step 10 conventions. Each row
is `[OK]/[FIX]/[WARN]/[ERR]`. Reuse `pbrain doctor --json --fast` for
informational rows but DO NOT gate the guidance block on doctor (per
perfectworld-plan-eng-review §6 — doctor is too strict for unrelated reasons).
...
pbrain status: GREEN
  ...
  CWD source ...... OK   <perfectworld-code-{repo_slug}> (page_count=<N>)
  Call graph ...... OK   <N> edges resolved (code-callers/callees live)
  ~/.perfectworld source. OK   <perfectworld-brain-{user}> (page_count=<N>) — managed by perfectworld-setup-pbrain
  Memory sync ..... OK   <artifacts_sync_mode>
  AGENTS.md or repository guidance ....... OK   ## PBrain Search Guidance present
  Last sync ....... OK   <last_sync from state file>

Run `perfectworld-sync-pbrain` again any time pbrain feels off; safe and idempotent.
```
...
1. **If a dream stage ran this invocation** (`--dream`, or `--full` auto-build),
   ...
   - `WARN dream ran but this source's schema pack does not extract code symbols
     — switch to a code-aware pack (\`pbrain schema use <pack>\`)`
...
2. **Otherwise** fall back to the `CYCLE` value from Step 3.5, with honest wording
   ...
   - `never` → `WARN call graph not built — run perfectworld-sync-pbrain --dream`
   - `unknown` → `WARN could not probe call graph (doctor unavailable) — run perfectworld-sync-pbrain --dream if code-callers returns 0`
...
check failed; AGENTS.md or repository guidance guidance block REMOVED — run perfectworld-setup-pbrain to repair`).
...
Mac. The orchestrator acquires a lock at `~/.perfectworld/.sync-pbrain.lock` before
any state-file or AGENTS.md or repository guidance mutation and exits with code 2 if another sync is
in flight. Stale locks (process died) auto-clear after 5 minutes.

## Cross-machine note

The `## PBrain Search Guidance` block is committed to the repo's AGENTS.md or repository guidance
and travels with `git push`/`git pull` — NOT through `~/.perfectworld/.brain-allowlist`
(which is for `~/.perfectworld/` brain-sync only). On a different Mac with a synced
AGENTS.md or repository guidance but no local pbrain, perfectworld-sync-pbrain detects the mismatch via the
...
- **DONE** — all stages green, AGENTS.md or repository guidance guidance block present, verdict GREEN.
- **DONE_WITH_CONCERNS** — sync ran but at least one stage failed or capability
  check failed. List which.
- **BLOCKED** — could not acquire lock, pbrain not on PATH, or per-repo policy
  is deny. State the blocker.
- **NEEDS_CONTEXT** — perfectworld-setup-pbrain has not been run, or `pbrain doctor` shows
