...
preamble-tier: 2
version: 1.0.0
description: Post-deploy canary monitoring. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"canary","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"canary","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"canary","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-canary — Post-Deploy Visual Monitor
...
## User-invocable
When the user types `perfectworld-canary`, run this skill.

## Arguments
- `perfectworld-canary <url>` — monitor a URL for 10 minutes after deploy
- `perfectworld-canary <url> --duration 5m` — custom monitoring duration (1m to 30m)
- `perfectworld-canary <url> --baseline` — capture baseline screenshots (run BEFORE deploying)
- `perfectworld-canary <url> --pages /,perfectworld-dashboard,perfectworld-settings` — specify pages to monitor
- `perfectworld-canary <url> --quick` — single-pass health check (no continuous monitoring)
...
mkdir -p .perfectworld/canary-reports
mkdir -p .perfectworld/canary-reports/baselines
mkdir -p .perfectworld/canary-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".perfectworld/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.perfectworld/canary-reports/baseline.json`:
...
Then STOP and tell the user: "Baseline captured. Deploy your changes, then run `perfectworld-canary <url>` to monitor."
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".perfectworld/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".perfectworld/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
─────────────────────────────────────────────────────
  ...
  /               HEALTHY     0         450ms
  perfectworld-dashboard      DEGRADED    2 new     1200ms (was 400ms)
  perfectworld-settings       HEALTHY     0         380ms

Alerts Fired:  [N] (X critical, Y high, Z medium)
Screenshots:   .perfectworld/canary-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.perfectworld/canary-reports/{date}-canary.md` and `.perfectworld/canary-reports/{date}-canary.json`.
