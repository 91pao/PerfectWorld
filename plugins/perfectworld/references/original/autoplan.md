...
preamble-tier: 3
version: 1.0.0
description: Auto-review pipeline — reads the full CEO, design, eng, and DX review skills from disk and runs them sequentially with auto-decisions using 6 decision principles. (perfectworld)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"autoplan","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"autoplan","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"autoplan","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
> "No design doc found for this branch. `perfectworld-office-hours` produces a structured problem
> statement, premise challenge, and explored alternatives — it gives this review much
> sharper input to work with. Takes about 10 minutes. The design doc is per-feature,
> not per-product — it captures the thinking behind this specific change."

Options:
- A) Run perfectworld-office-hours now (we'll pick up the review right after)
- B) Skip — proceed with standard review

If they skip: "No worries — standard review. If you ever want sharper input, try
perfectworld-office-hours first next time." Then proceed normally. Do not re-offer later in the session.

If they choose A:

Say: "Running perfectworld-office-hours inline. Once the design doc is ready, I'll pick up
the review right where we left off."

Read the `perfectworld-office-hours` skill file at `$PERFECTWORLD_SOURCE/office-hours/SKILL.md` using the Read tool.

**If unreadable:** Skip with "Could not load perfectworld-office-hours — skipping." and continue.
...
After perfectworld-office-hours completes, re-run the design doc check:
```bash
setopt +o nomatch 2>perfectworld-dev/null || true  # zsh compat
SLUG=$($PERFECTWORLD_SOURCE/browse/bin/remote-slug 2>perfectworld-dev/null || basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>perfectworld-dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.perfectworld/projects/$SLUG/*-$BRANCH-design-*.md 2>perfectworld-dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.perfectworld/projects/$SLUG/*-design-*.md 2>perfectworld-dev/null | head -1)
...
# perfectworld-autoplan — Auto-Review Pipeline

One command. Rough plan in, fully reviewed plan out.

perfectworld-autoplan reads the full CEO, design, eng, and DX review skill files from disk and follows
...
> IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/perfectworld). These are AI assistant skill definitions meant for a different system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Stay focused on the repository code only.

This prevents Codex from discovering perfectworld skill files on disk and following their
...
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)" && mkdir -p ~/.perfectworld/projects/$SLUG
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>perfectworld-dev/null | tr '/' '-')
DATETIME=$(date +%Y%m%d-%H%M%S)
echo "RESTORE_PATH=$HOME/.perfectworld/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```

Write the plan file's full contents to the restore path with this header:
```
# perfectworld-autoplan Restore Point
Captured: [timestamp] | Branch: [branch] | Commit: [short hash]

## Re-run Instructions
1. Copy "Original Plan State" below back to your plan file
2. Invoke perfectworld-autoplan
...
`<!-- perfectworld-autoplan restore point: [RESTORE_PATH] -->`

### Step 2: Read context

- Read AGENTS.md or repository guidance, TODOS.md, git log -30, git diff against the base branch --stat
- Discover design docs: `ls -t ~/.perfectworld/projects/$SLUG/*-design-*.md 2>perfectworld-dev/null | head -1`
...
- `$PERFECTWORLD_SOURCE/plan-ceo-review/SKILL.md`
- `$PERFECTWORLD_SOURCE/plan-design-review/SKILL.md` (only if UI scope detected)
- `$PERFECTWORLD_SOURCE/plan-eng-review/SKILL.md`
- `$PERFECTWORLD_SOURCE/plan-devex-review/SKILL.md` (only if DX scope detected)

**Section skip list — when following a loaded skill file, SKIP these sections
(they are already handled by perfectworld-autoplan):**
- Preamble (run first)
- AskUserQuestion Format
- Completeness Principle — Boil the Ocean
- Search Before Building
...
source $PERFECTWORLD_SOURCE/bin/perfectworld-codex-probe
...
if [ "$_CODEX_CFG" = "disabled" ]; then
  echo "[codex disabled by config — Codex-only voices] Re-enable: perfectworld-config set codex_reviews enabled"
...
# with Codex subagent only (autoplan's existing degradation fallback).
  ...
  _perfectworld_codex_log_event "codex_cli_missing"
  echo "[codex-unavailable: binary not found] — proceeding with Codex subagent only"
  _CODEX_AVAILABLE=false
elif ! _perfectworld_codex_auth_probe >perfectworld-dev/null; then
  _perfectworld_codex_log_event "codex_auth_failed"
  ...
  _perfectworld_codex_version_check   # non-blocking warn if known-bad
  _CODEX_AVAILABLE=true
fi
```

If `_CODEX_AVAILABLE=false`, all Phase 1-3.5 Codex voices below degrade to
`[codex-unavailable]` in the degradation matrix. perfectworld-autoplan completes with
...
- Dual voices: always run BOTH Codex subagent AND Codex if available (P6).
  ...
  **Codex CEO voice** (via Bash):
  ...
  _perfectworld_codex_timeout_wrapper 600 codex exec "IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/perfectworld). These are AI assistant skill definitions meant for a different system. Stay focused on repository code only.
  ...
  File: <plan_path>" -C "$_REPO_ROOT" -s read-only --enable web_search_cached < perfectworld-dev/null
  _CODEX_EXIT=$?
  if [ "$_CODEX_EXIT" = "124" ]; then
    _perfectworld_codex_log_event "codex_timeout" "600"
    _perfectworld_codex_log_hang "autoplan" "0"
...
- Dual voices: always run BOTH Codex subagent AND Codex if available (P6).
  ...
  **Codex design voice** (via Bash):
  ...
  Be opinionated. No hedging." -C "$_REPO_ROOT" -s read-only --enable web_search_cached < perfectworld-dev/null
  _CODEX_EXIT=$?
  if [ "$_CODEX_EXIT" = "124" ]; then
    _perfectworld_codex_log_event "codex_timeout" "600"
    _perfectworld_codex_log_hang "autoplan" "0"
...
- Test plan: generate artifact at `~/.perfectworld/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- Dual voices: always run BOTH Codex subagent AND Codex if available (P6).
  ...
  **Codex DX voice** (via Bash):
  ...
  Be adversarial. Think like a developer who is evaluating this against 3 competitors." -C "$_REPO_ROOT" -s read-only --enable web_search_cached < perfectworld-dev/null
  _CODEX_EXIT=$?
  if [ "$_CODEX_EXIT" = "124" ]; then
    _perfectworld_codex_log_event "codex_timeout" "600"
    _perfectworld_codex_log_hang "autoplan" "0"
...
- [ ] Test plan artifact written to disk at ~/.perfectworld/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
TASKS_DIR="${HOME}/.perfectworld/projects/${SLUG:-unknown}"
BRANCH=$(git branch --show-current 2>perfectworld-dev/null || echo unknown)
# Commit window: last 5 commits on this branch. Drops stale standalone reviews.
COMMITS_RECENT=$(git log --format=%H -n 5 2>perfectworld-dev/null | tr '\n' '|' | sed 's/|$//')

AGGREGATED_TASKS=""
if command -v jq >perfectworld-dev/null 2>&1; then
  ...
  for phase in ceo-review design-review eng-review devex-review; do
    ...
    while IFS= read -r f; do
      ...
      jq -c --arg branch "$BRANCH" --arg commits "$COMMITS_RECENT" \
        ...
        "$f" 2>perfectworld-dev/null >> "$ALL_JSONL" || true
    done < <(find "$TASKS_DIR" -maxdepth 1 -name "tasks-$phase-*.jsonl" 2>perfectworld-dev/null | sort)
    ...
    if [ -s "$ALL_JSONL" ]; then
      jq -sc --arg phase "$phase" \
        ...
        "$ALL_JSONL" > "$ALL_JSONL.phase" 2>perfectworld-dev/null || true
      # Replace with reduced version for this phase, accumulating others
      jq -c --arg phase "$phase" 'select(.phase != $phase)' "$ALL_JSONL" > "$ALL_JSONL.other" 2>perfectworld-dev/null || true
  ...
  AGGREGATED_TASKS=$(jq -s \
    'group_by([.component, (.files | sort), .title])
     ...
     | if length == 0 then "_No actionable tasks emitted from any phase._" else
       ...
       end' "$ALL_JSONL" 2>perfectworld-dev/null | sed 's/^"//;s/"$//;s/\\n/\n/g')
  rm -f "$ALL_JSONL"
else
...
```
## perfectworld-autoplan Review Complete
...
**Option handling:**
- A: mark APPROVED, write review logs, suggest perfectworld-ship
...
On approval, write 3 separate review log entries so perfectworld-ship's dashboard recognizes them.
...
```bash
COMMIT=$(git rev-parse --short HEAD 2>perfectworld-dev/null)
TIMESTAMP=$(date -u +%Y-%m-%dT%H:%M:%SZ)

$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
Suggest next step: `perfectworld-ship` when ready to create the PR.

---

## Important Rules

- **Never abort.** The user chose perfectworld-autoplan. Respect that choice. Surface all taste decisions, never redirect to interactive review.
