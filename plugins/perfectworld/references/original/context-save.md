...
preamble-tier: 2
version: 1.0.0
description: Save working context. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"context-save","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"context-save","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"context-save","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-context-save — Save Working Context
...
can resume without losing a beat via `perfectworld-context-restore`.
...
- `perfectworld-context-save` or `perfectworld-context-save <title>` → **Save**
- `perfectworld-context-save list` → **List**

If the user provides a title after the command (e.g., `perfectworld-context-save auth refactor`),
use it as the title. Otherwise, infer a title from the current work.

If the user types `perfectworld-context-save resume` or `perfectworld-context-save restore`, tell them:
"Use `perfectworld-context-restore` instead — save and restore are separate skills now."
...
```bash
echo "=== BRANCH ==="
git rev-parse --abbrev-ref HEAD 2>perfectworld-dev/null
echo "=== STATUS ==="
git status --short 2>perfectworld-dev/null
echo "=== DIFF STAT ==="
git diff --stat 2>perfectworld-dev/null
echo "=== STAGED DIFF STAT ==="
git diff --cached --stat 2>perfectworld-dev/null
echo "=== RECENT LOG ==="
git log --oneline -10 2>perfectworld-dev/null
```
...
if [ -n "$_TEL_START" ]; then
  ...
  START_EPOCH=$(ps -o lstart= -p $PPID 2>perfectworld-dev/null | xargs -I{} date -jf "%c" "{}" "+%s" 2>perfectworld-dev/null || echo "")
fi
...
CHECKPOINT_DIR="$PERFECTWORLD_STATE_ROOT/projects/$SLUG/checkpoints"
...
if [ -e "$FILE" ]; then
  SUFFIX=$(LC_ALL=C tr -dc 'a-z0-9' < perfectworld-dev/urandom 2>perfectworld-dev/null | head -c 4 || printf '%04x' "$$")
...
Restore later with perfectworld-context-restore.
```
...
if [ -d "$CHECKPOINT_DIR" ]; then
  ...
  find "$CHECKPOINT_DIR" -maxdepth 1 -name "*.md" -type f 2>perfectworld-dev/null | sort -r
else
  echo "NO_CHECKPOINTS"
fi
```
...
If the user passes `--all` (e.g., `perfectworld-context-save list --all`), show contexts
from **all branches**.
...
`perfectworld-context-save` to save your current working state."
...
- **This is a perfectworld skill, not a Codex built-in.** When the user types
  `perfectworld-context-save`, use this skill via the Skill tool. The old `perfectworld-checkpoint`
  name collided with Codex's native `perfectworld-rewind` alias — the rename fixed that.