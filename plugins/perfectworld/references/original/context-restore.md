...
preamble-tier: 2
version: 1.0.0
description: Restore working context saved earlier by perfectworld-context-save. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Glob
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"context-restore","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"context-restore","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
# perfectworld-context-restore — Restore Saved Working Context
...
intentionally different from `perfectworld-context-save list`, which defaults to the current
branch. `perfectworld-context-restore` is for Conductor workspace handoff — a context saved
on one branch can be resumed from another.

**Do NOT filter the candidate set by current branch.** The `list` flow does
that; `perfectworld-context-restore` does not.
...
- `perfectworld-context-restore` → load the most recent saved context (any branch)
- `perfectworld-context-restore <title-fragment-or-number>` → load a specific saved context
- `perfectworld-context-restore list` → tell the user "Use `perfectworld-context-save list` — listing
...
if [ ! -d "$CHECKPOINT_DIR" ]; then
  ...
  # context window just listing them. perfectworld-context-save list handles pagination.
  FILES=$(find "$CHECKPOINT_DIR" -maxdepth 1 -name "*.md" -type f 2>perfectworld-dev/null | sort -r | head -20)
  if [ -z "$FILES" ]; then
    echo "NO_CHECKPOINTS"
  else
    echo "$FILES"
...
"No saved contexts yet. Run `perfectworld-context-save` first to save your current working
state, then `perfectworld-context-restore` will find it."
...
- **This is a perfectworld skill, not a Codex built-in.** When the user types
  `perfectworld-context-restore`, use this skill via the Skill tool.