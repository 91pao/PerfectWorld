...
preamble-tier: 3
version: 2.0.0
description: YC Office Hours — two modes. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Grep
...
pbrain:
  ...
  context_queries:
    ...
    - id: builder-profile
      kind: filesystem
      glob: "~/.perfectworld/builder-profile.jsonl"
      tail: 1
    ...
    - id: prior-eureka
      kind: filesystem
      glob: "~/.perfectworld/analytics/eureka.jsonl"
      tail: 5
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"office-hours","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"office-hours","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)" 2>perfectworld-dev/null || true
{
  ...
  printf '\n### %s\n\n' "salience"
  $PERFECTWORLD_SOURCE/bin/perfectworld-brain-cache get salience --project "$SLUG" 2>perfectworld-dev/null || printf '_(no salience digest available yet)_\n'
...
1. Read `AGENTS.md or repository guidance`, `TODOS.md` (if they exist).
2. Run `git log --oneline -30` and `git diff origin/main --stat 2>perfectworld-dev/null` to understand recent context.
3. Use Grep/Glob to map the codebase areas most relevant to the user's request.
4. **List existing design docs for this project:**
   ```bash
   setopt +o nomatch 2>perfectworld-dev/null || true  # zsh compat
   ls -t ~/.perfectworld/projects/$SLUG/*-design-*.md 2>perfectworld-dev/null
   ```
...
```bash
setopt +o nomatch 2>perfectworld-dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.perfectworld/projects/$SLUG/*-design-*.md 2>perfectworld-dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.perfectworld/projects/`.
...
After understanding the problem through questioning, search for what the world thinks. This is NOT competitive research (that's perfectworld-design-consultation's job). This is understanding conventional wisdom so you can evaluate where it's wrong.
...
```bash
CODEX_PROMPT_FILE=$(mktemp perfectworld-tmp/perfectworld-codex-oh-XXXXXXXX.txt)
```
...
```bash
TMPERR_OH=$(mktemp perfectworld-tmp/codex-oh-err-XXXXXXXX)
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
codex exec "$(cat "$CODEX_PROMPT_FILE")" -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' --enable web_search_cached < perfectworld-dev/null 2>"$TMPERR_OH"
```
...
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
_DESIGN_DIR="$HOME/.perfectworld/projects/$SLUG/designs/mockup-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
4. POST the new HTML to the running board. Parse the board URL from stderr
   ...
   `--no-daemon`, hits `perfectworld-api/reload` root). Daemon path:
...
```bash
echo '{"approved_variant":"<VARIANT>","feedback":"<FEEDBACK>","date":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","screen":"mockup","branch":"'$(git branch --show-current 2>perfectworld-dev/null)'"}' > "$_DESIGN_DIR/approved.json"
```
...
```bash
SKETCH_FILE="perfectworld-tmp/perfectworld-sketch-$(date +%s).html"
```
...
$B screenshot perfectworld-tmp/perfectworld-sketch.png
```
...
The screenshot file at `perfectworld-tmp/perfectworld-sketch.png` can be referenced by downstream skills
(`perfectworld-plan-design-review`, `perfectworld-design-review`) to see what was originally envisioned.
...
```bash
TMPERR_SKETCH=$(mktemp perfectworld-tmp/codex-sketch-XXXXXXXX)
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
codex exec "For this product approach, provide: a visual thesis (one sentence — mood, material, energy), a content plan (hero → support → detail → CTA), and 2 interaction ideas that change page feel. Apply beautiful defaults: composition-first, brand-first, cardless, poster not document. Be opinionated." -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' --enable web_search_cached < perfectworld-dev/null 2>"$TMPERR_SKETCH"
```
...
`perfectworld-developer-profile --log-session` binary handles its own directory creation
and writes via atomic mktemp+mv to `~/.perfectworld/developer-profile.json`.
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>perfectworld-dev/null || true
```
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `$PERFECTWORLD_SOURCE/office-hours/sections/design-and-handoff.md` and execute it
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
