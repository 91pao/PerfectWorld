...
preamble-tier: 2
version: 2.0.0
description: Weekly engineering retrospective. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
pbrain:
  ...
  context_queries:
    - id: prior-retros
      kind: filesystem
      glob: "~/.perfectworld/projects/{repo_slug}perfectworld-retros/*.md"
      sort: mtime_desc
      limit: 5
      render_as: "## Prior retros for this project"
    - id: recent-timeline
      kind: filesystem
      glob: "~/.perfectworld/projects/{repo_slug}perfectworld-timeline.jsonl"
      tail: 30
      render_as: "## Recent timeline events"
    - id: recent-learnings
      kind: filesystem
      glob: "~/.perfectworld/projects/{repo_slug}perfectworld-learnings.jsonl"
      tail: 10
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"retro","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-retro — Weekly Engineering Retrospective
...
## User-invocable
When the user types `perfectworld-retro`, run this skill.

## Arguments
- `perfectworld-retro` — default: last 7 days
- `perfectworld-retro 24h` — last 24 hours
- `perfectworld-retro 14d` — last 14 days
- `perfectworld-retro 30d` — last 30 days
- `perfectworld-retro compare` — compare current window vs prior same-length window
- `perfectworld-retro compare 14d` — compare with explicit window
- `perfectworld-retro global` — cross-project retro across all AI coding tools (7d default)
- `perfectworld-retro global 14d` — cross-project retro with explicit window
...
```
Usage: perfectworld-retro [window | compare | global]
  perfectworld-retro              — last 7 days (default)
  perfectworld-retro 24h          — last 24 hours
  perfectworld-retro 14d          — last 14 days
  perfectworld-retro 30d          — last 30 days
  perfectworld-retro compare      — compare this period vs prior period
  perfectworld-retro compare 14d  — compare with explicit window
  perfectworld-retro global       — cross-project retro across all AI tools (7d default)
  perfectworld-retro global 14d   — cross-project retro with explicit window
```
...
```bash
[ -f ~/.perfectworld/retro-context.md ] && echo "RETRO_CONTEXT_FOUND" || echo "NO_RETRO_CONTEXT"
```

If `RETRO_CONTEXT_FOUND`: read `~/.perfectworld/retro-context.md`. This file is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate this context into the retro narrative where relevant.
...
_RETRO_HAS_REMOTE=$(git remote 2>perfectworld-dev/null | grep -c '^origin$' || echo 0)
...
if [ -z "$_RETRO_GUARD_VERDICT" ]; then
  _RETRO_HEAD_REF=$(git symbolic-ref --quiet HEAD 2>perfectworld-dev/null || echo "")
...
if [ -z "$_RETRO_GUARD_VERDICT" ]; then
  if ! git fetch origin <default> --quiet 2>perfectworld-dev/null; then
...
if [ -z "$_RETRO_GUARD_VERDICT" ]; then
  _RETRO_LATEST_ISO=$(git log -1 --format=%ci origin/<default> 2>perfectworld-dev/null | awk '{print $1}')
...
cat ~/.perfectworld/greptile-history.md 2>perfectworld-dev/null || true

# 9. TODOS.md backlog (if available)
cat TODOS.md 2>perfectworld-dev/null || true

# 10. Test file count
find . -name '*.test.*' -o -name '*.spec.*' -o -name '*_test.*' -o -name '*_spec.*' 2>perfectworld-dev/null | grep -v node_modules | wc -l

# 11. Regression test commits in window
git log origin/<default> --since="<window>" --oneline --grep="test(qa):" --grep="test(design):" --grep="test: coverage"

# 12. perfectworld skill usage telemetry (if available)
cat ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
...
**Greptile signal (if history exists):** Read `~/.perfectworld/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `~/.perfectworld/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:

```
| Skill Usage | perfectworld-ship(12) perfectworld-qa(8) perfectworld-review(5) · 3 safety hook fires |
```

If the JSONL file doesn't exist or has no entries in the window, skip the Skill Usage row.

**Eureka Moments (if logged):** Read `~/.perfectworld/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```
  EUREKA perfectworld-office-hours (branch: garrytan/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
  EUREKA perfectworld-plan-eng-review (branch: garrytan/cache-layer): "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
ls -t .context/retros/*.json 2>perfectworld-dev/null
```
...
today=$(date +%Y-%m-%d)
existing=$(ls .context/retros/${today}-*.json 2>perfectworld-dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
...
**Note:** Only include the `greptile` field if `~/.perfectworld/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
### Plan Completion
Check review JSONL logs for plan completion data from perfectworld-ship runs this period:

```bash
setopt +o nomatch 2>perfectworld-dev/null || true  # zsh compat
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
cat ~/.perfectworld/projects/$SLUG/*-reviews.jsonl 2>perfectworld-dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
When the user runs `perfectworld-retro global` (or `perfectworld-retro global 14d`), follow this flow instead of the repo-scoped Steps 1-14. This mode works from any directory — it does NOT require being inside a git repo.
...
```bash
DISCOVER_BIN=""
[ -x $PERFECTWORLD_SOURCE/bin/perfectworld-global-discover ] && DISCOVER_BIN=$PERFECTWORLD_SOURCE/bin/perfectworld-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x .agents/skills/perfectworld/bin/perfectworld-global-discover ] && DISCOVER_BIN=.agents/skills/perfectworld/bin/perfectworld-global-discover
[ -z "$DISCOVER_BIN" ] && which perfectworld-global-discover >perfectworld-dev/null 2>&1 && DISCOVER_BIN=$(which perfectworld-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/perfectworld-global-discover.ts ] && DISCOVER_BIN="bun run bin/perfectworld-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the perfectworld directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>perfectworld-tmp/perfectworld-discover-stderr
```

Read the stderr output from `perfectworld-tmp/perfectworld-discover-stderr` for diagnostic info. Parse the JSON output from stdout.

If `total_sessions` is 0, say: "No AI coding sessions found in the last <window>. Try a longer window: `perfectworld-retro global 30d`" and stop.
...
```bash
git -C <path> fetch origin --quiet 2>perfectworld-dev/null
```
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  E.g., "Built perfectworld-retro global — cross-project retrospective with AI session discovery"
  not "feat: perfectworld-global-discover" + "feat: perfectworld-retro global template".
...
```bash
setopt +o nomatch 2>perfectworld-dev/null || true  # zsh compat
ls -t ~/.perfectworld/retros/global-*.json 2>perfectworld-dev/null | head -5
```
...
setopt +o nomatch 2>perfectworld-dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.perfectworld/retros/global-${today}-*.json 2>perfectworld-dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.perfectworld/retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: perfectworld (58%) | Streak: 52d"
}
```
...
When the user runs `perfectworld-retro compare` (or `perfectworld-retro compare 14d`):
...
- Do not read AGENTS.md or repository guidance or other docs — this skill is self-contained
- On first run (no prior retros), skip comparison sections gracefully
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.perfectworld/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".