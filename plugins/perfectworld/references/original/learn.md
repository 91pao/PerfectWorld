...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"learn","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"learn","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"learn","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
see what perfectworld has learned across sessions on this project, search for relevant
...
- `perfectworld-learn` (no arguments) → **Show recent**
- `perfectworld-learn search <query>` → **Search**
- `perfectworld-learn prune` → **Prune**
- `perfectworld-learn export` → **Export**
- `perfectworld-learn stats` → **Stats**
- `perfectworld-learn add` → **Manual add**
...
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-search --limit 20 2>perfectworld-dev/null || echo "No learnings yet."
```

Present the output in a readable format. If no learnings exist, tell the user:
"No learnings recorded yet. As you use perfectworld-review, perfectworld-ship, perfectworld-investigate, and other skills,
perfectworld will automatically capture patterns, pitfalls, and insights it discovers."
...
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-search --query "USER_QUERY" --limit 20 2>perfectworld-dev/null || echo "No matches."
```
...
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-search --limit 100 2>perfectworld-dev/null
```
...
Export learnings as markdown suitable for adding to AGENTS.md or repository guidance or project documentation.

```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-search --limit 50 2>perfectworld-dev/null
```
...
Present the formatted output to the user. Ask if they want to append it to AGENTS.md or repository guidance
...
LEARN_FILE="$PERFECTWORLD_STATE_ROOT/projects/$SLUG/learnings.jsonl"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```