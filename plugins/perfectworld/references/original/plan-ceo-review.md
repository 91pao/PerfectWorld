...
interactive: true
version: 1.0.0
description: CEO/founder-mode plan review. (perfectworld)
...
pbrain:
  ...
  context_queries:
    - id: prior-ceo-plans
      kind: filesystem
      glob: "~/.perfectworld/projects/{repo_slug}perfectworld-ceo-plans/*.md"
      sort: mtime_desc
      limit: 5
      render_as: "## Prior CEO plans for this project"
    - id: recent-design-docs
      kind: filesystem
      glob: "~/.perfectworld/projects/{repo_slug}/*-design-*.md"
      sort: mtime_desc
      limit: 3
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"plan-ceo-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```
Then read AGENTS.md or repository guidance, TODOS.md, and any existing architecture docs.
...
```
If a design doc exists (from `perfectworld-office-hours`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.

**Handoff note check** (reuses $SLUG and $BRANCH from the design doc check above):
```bash
setopt +o nomatch 2>perfectworld-dev/null || true  # zsh compat
HANDOFF=$(ls -t ~/.perfectworld/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>perfectworld-dev/null | head -1)
...
from a prior CEO review session that paused so the user could run `perfectworld-office-hours`. Use it
...
sure," or is clearly exploring rather than reviewing — offer `perfectworld-office-hours`:

> "It sounds like you're still figuring out what to build — that's totally fine, but
> that's what perfectworld-office-hours is designed for. Want to run perfectworld-office-hours right now?
> We'll pick up right where we left off."

Options: A) Yes, run perfectworld-office-hours now. B) No, keep going.
...
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)" 2>perfectworld-dev/null || true
{
  ...
  printf '\n### %s\n\n' "product"
  $PERFECTWORLD_SOURCE/bin/perfectworld-brain-cache get product --project "$SLUG" 2>perfectworld-dev/null || printf '_(no product digest available yet)_\n'
  printf '\n### %s\n\n' "goals"
  $PERFECTWORLD_SOURCE/bin/perfectworld-brain-cache get goals --project "$SLUG" 2>perfectworld-dev/null || printf '_(no goals digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  $PERFECTWORLD_SOURCE/bin/perfectworld-brain-cache get recent-decisions --project "$SLUG" 2>perfectworld-dev/null || printf '_(no recent-decisions digest available yet)_\n'
  printf '\n### %s\n\n' "user-profile"
  $PERFECTWORLD_SOURCE/bin/perfectworld-brain-cache get user-profile  2>perfectworld-dev/null || printf '_(no user-profile digest available yet)_\n'
} > perfectworld-tmp/.perfectworld-brain-context-$$.md 2>perfectworld-dev/null
[ -s perfectworld-tmp/.perfectworld-brain-context-$$.md ] && cat perfectworld-tmp/.perfectworld-brain-context-$$.md
rm -f perfectworld-tmp/.perfectworld-brain-context-$$.md 2>perfectworld-dev/null || true
```
...
`perfectworld/`, `concepts/` only). Personal/family/therapy content never leaks here.
...
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)" && mkdir -p ~/.perfectworld/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.perfectworld/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.perfectworld/projects/$SLUG/ceo-plans/{old-plan}.md ~/.perfectworld/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.perfectworld/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
Generated by perfectworld-plan-ceo-review on {date}
...
```bash
mkdir -p ~/.perfectworld/analytics
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.perfectworld/analytics/spec-review.jsonl 2>perfectworld-dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + perfectworld,
...
> **STOP.** Before running the 11-section deep review, required outputs, and review report (only after Step 0 scope and mode are agreed), Read `$PERFECTWORLD_SOURCE/plan-ceo-review/sections/review-sections.md` and execute it
...
2. Confirm the LAST `## ` heading in the file is `## PERFECTWORLD REVIEW REPORT`.
   In-body prose that mentions "outside voice", "codex findings", or similar
   does NOT count — only the structured `## PERFECTWORLD REVIEW REPORT` section
...
5. If a plan file is in context for this skill invocation: confirm
   `perfectworld-review-log` was called and `perfectworld-review-read` was run at least
   once. If no plan file is in context (e.g. `perfectworld-codex consult` against a
