...
preamble-tier: 3
version: 1.0.0
description: Live-device iOS QA for SwiftUI apps. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ios-qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"ios-qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"ios-qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
```
       ┌──────────────────────┐   USB CoreDevice (IPv6)   ┌──────────────────┐
       │ perfectworld-ios-qa daemon │ ────────────────────────▶ │ iOS app          │
       │ (Mac, bun/TS)        │   bearer + X-Session-Id   │ StateServer      │
       │                      │                           │ (loopback only)  │
       │ - boot token rotate  │                           │ - perfectworld-tap perfectworld-swipe    │
       │ - session minting    │                           │ - perfectworld-type perfectworld-state   │
       │ - audit + redact     │                           │ - perfectworld-snapshot      │
       └──────────────────────┘                           └──────────────────┘
                ▲
...
If `~/.perfectworld/ios-qa-session.json` exists and the device is still connected,
...
```bash
SESSION="$HOME/.perfectworld/ios-qa-session.json"
if [ -f "$SESSION" ] && [ "$COLD" != "1" ]; then
  ...
  if curl -sf "http://127.0.0.1:$CACHED_PORT/healthz" > perfectworld-dev/null; then
...
2. Run `swift run --package-path $PERFECTWORLD_HOME/ios-qa/scripts/gen-accessors-tool gen-accessors --input <source-dir>`.
...
5. Spawn the Mac-side daemon (on-demand) — `perfectworld-ios-qa-daemon`. Daemon
   acquires an exclusive flock on `~/.perfectworld/ios-qa-daemon.pid`. If another
   daemon is alive, the second invocation discovers its port and connects.
6. Daemon immediately calls `POST perfectworld-auth/rotate` on the iOS StateServer with a
...
1. `GET perfectworld-screenshot` (via daemon) → save PNG.
2. `GET perfectworld-elements` → accessibility tree.
3. `GET perfectworld-state/snapshot` (only `@Snapshotable` fields) → current state.
4. Decide next action based on what's on the screen vs the test goal.
5. `POST perfectworld-session/acquire` to grab the device lock.
6. Execute `POST perfectworld-tap`, `perfectworld-swipe`, `perfectworld-type`, or `POST perfectworld-state/<key>` write.
7. Re-screenshot; compare; record finding if buggy.
8. `POST perfectworld-session/release` once the iteration is done.

Each authenticated mutating request through the tailnet listener (if remote
mode is active) writes an audit row to
`~/.perfectworld/security/ios-qa-audit.jsonl`.
...
the daemon to be able to read `perfectworld-var/run/tailscale.sock`. Fails closed if the
socket is missing, permission-denied, or returns an unparseable WhoIs
response. Remote agents hit `POST perfectworld-auth/mint` over tailnet, daemon
...
- **observe:** `perfectworld-screenshot`, `perfectworld-elements`, `GET perfectworld-state/*`, `perfectworld-healthz`,
  `perfectworld-session/heartbeat`.
- **interact:** observe + `perfectworld-tap`, `perfectworld-swipe`, `perfectworld-type`.
- **mutate:** interact + `POST perfectworld-state/<key>`.
- **restore:** mutate + `POST perfectworld-state/restore`.

Owner mints via `perfectworld-ios-qa-mint --remote <identity> --capability <tier>`
...
agent MUST drive every action through visible UI (`perfectworld-tap`, `perfectworld-swipe`, `perfectworld-type`)
and NEVER use `POST perfectworld-state/*` writes to skip steps. Viewers see the agent
type every key, tap every button. The on-device DebugOverlay attribution
chip shows "Driven by Codex (demo)" or the remote agent identity.
...
|---|---|---|
| `curl: connection refused` to daemon | daemon crashed | Re-run `perfectworld-ios-qa`; spawn-race lock will fail closed |
| `403 identity_not_allowed` from `perfectworld-auth/mint` | identity missing from allowlist | Run `perfectworld-ios-qa-mint --remote <identity>` on the Mac |
| `409 schema_mismatch` on `perfectworld-state/restore` | snapshot from older app build | Discard the snapshot; re-capture |
| `503 device_disconnected` from proxy | USB tunnel dropped | Reconnect device; daemon auto-reconnects within 30s |
| `429 rate_limited` from `perfectworld-auth/mint` | >10 mints/min from one identity | Wait 60s; check audit log for anomalies |
| `413 body_too_large` on `perfectworld-state/restore` | snapshot >1MB | Increase `--max-body` or trim snapshot |

## Cleanup

Use `perfectworld-ios-clean` to remove the DebugBridge SPM dependency and all `#if DEBUG`
