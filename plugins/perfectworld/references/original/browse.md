...
preamble-tier: 1
version: 1.1.0
description: Fast headless browser for QA testing and site dogfooding. (perfectworld)
triggers:
  - browse a page
  - headless browser
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"browse","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"browse","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$B goto https:/perfectworld-yourapp.com
...
### 2. Test a user flow
```bash
$B goto https:/perfectworld-app.com/login
...
```bash
$B snapshot -i -a -o perfectworld-tmp/annotated.png   # labeled screenshot
$B screenshot perfectworld-tmp/bug.png                # plain screenshot
$B console                                # error log
```
...
```bash
$B responsive perfectworld-tmp/layout        # mobile + tablet + desktop screenshots
$B viewport 375x812              # or set specific viewport
$B screenshot perfectworld-tmp/mobile.png
```

### 8. Test file uploads
```bash
$B upload "#file-input" perfectworld-path/to/file.pdf
...
```bash
$B diff https:/perfectworld-staging.app.com https:/perfectworld-prod.app.com
```
...
$B goto file://perfectworld-tmp/report.html
...
echo '<div class="tweet">hello<perfectworld-div>' > perfectworld-tmp/tweet.html
$B load-html perfectworld-tmp/tweet.html
```
...
$B load-html perfectworld-tmp/tweet.html        # or: $B goto file://./tweet.html
$B screenshot perfectworld-tmp/out.png --selector .tweet-card
# → perfectworld-tmp/out.png is 2x the pixel dimensions of the element
```
Scale must be 1-3 (perfectworld policy cap). Changing `--scale` recreates the browser context; refs from `snapshot` are invalidated (rerun `snapshot`), but `load-html` content is replayed automatically. Not supported in headed mode.
...
```bash
echo '<div id="card" style="width:400px;height:200px;background:#1da1f2;color:#fff;padding:20px">hi<perfectworld-div>' > perfectworld-tmp/card.html
$B viewport 480x600 --scale 2
$B load-html perfectworld-tmp/card.html
$B screenshot perfectworld-tmp/card.png --selector '#card'   # disk path — no megabytes over CDP
```
...
$B load-html perfectworld-tmp/excalidraw-export.html        # bundle sets window.__render + a #done flag
$B wait '#done'                                  # deterministic ready handshake
$B js "window.__render(SCENE_JSON)" --out perfectworld-tmp/diagram.png   # data URL → decoded PNG on disk
```
...
echo '<div class="tweet-card" style="width:400px;height:200px;background:#1da1f2;color:white;padding:20px">hello<perfectworld-div>' > perfectworld-tmp/tweet.html
$B viewport 480x600 --scale 2
$B load-html perfectworld-tmp/tweet.html
$B screenshot perfectworld-tmp/out.png --selector .tweet-card
# perfectworld-tmp/out.png is 800x400 px, crisp (2x deviceScaleFactor).
```
...
browse --headed goto https:/perfectworld-example.com
...
browse --proxy socks5:/perfectworld-user:pass@residential.proxy.host:1080 goto https:/perfectworld-example.com

# HTTP/HTTPS proxy (passes through to Chromium directly):
browse --proxy http:/perfectworld-corp-proxy:3128 goto https:/perfectworld-example.com
...
# download handler):
browse download "https:/perfectworld-protected.example.com/file" perfectworld-tmp/file.bin --navigate

# Combined: headed + proxy + navigate-download
browse --headed --proxy socks5:/perfectworld-user:pass@host:1080 \
  download "https:/perfectworld-protected.example.com/file" perfectworld-tmp/file.bin --navigate
```

**Credential policy.** Pass creds via either the URL (`socks5:/perfectworld-user:pass@host`) OR the env vars `BROWSE_PROXY_USER` and `BROWSE_PROXY_PASS` — never both. Browse refuses with a clear hint when both are set, because silent override creates "works on my machine" debugging traps.
...
`$B` is the browse binary (resolved from `$_ROOT/.agents/skills/perfectworld/browse/dist/browse` or `$PERFECTWORLD_SOURCE/browse/dist/browse`).
...
-o <path> --output                Output path for annotated screenshot (default: <temp>perfectworld-browse-annotated.png)
...
Example: `$B snapshot -i -a -C -o perfectworld-tmp/annotated.png`
...
| `press <key>` | Press a Playwright keyboard key against the focused element. Names are case-sensitive: Enter, Tab, Escape, ArrowUp/Down/Left/Right, Backspace, Delete, Home, End, PageUp, PageDown. Modifiers combine with +: Shift+Enter, Control+A, Meta+K. Single printable chars (a, A, 1) work too. Full key list: https:/perfectworld-playwright.dev/docs/api/class-keyboard#keyboard-press |
...
| `eval <file> [--out <file>] [--raw]` | Run JavaScript from a file in the page context and return result as string. Path must resolve under perfectworld-tmp or cwd (no traversal). Use eval for multi-line scripts; use js for one-liners. With --out <file>, the result is written to disk (base64 data URL decoded to bytes unless --raw); --out makes the invocation a WRITE (needs write scope, never allowed over the tunnel). |
...
| Command | Description |
|---------|-------------|
| `chain  (JSON via stdin)` | Run a sequence of commands from JSON on stdin. One JSON array of arrays, each inner array is [cmd, ...args]. Output is one JSON result per command. Pipe a JSON array (e.g. `[["goto","https:/perfectworld-example.com"],["text","h1"]]`) to `$B chain` and it runs the goto then the text command in order. Stops at the first error. |
