...
interactive: true
version: 2.0.0
description: Designer's eye plan review — interactive, like CEO and Eng review. (perfectworld)
allowed-tools:
  - Read
  - Edit
  - Grep
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"plan-design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
# perfectworld-plan-design-review: Designer's Eye Plan Review
...
### The perfectworld designer — YOUR PRIMARY TOOL

You have the **perfectworld designer**, an AI mockup generator that creates real visual mockups
...
- AGENTS.md or repository guidance — project conventions
...
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)" 2>perfectworld-dev/null || true
{
  ...
  printf '\n### %s\n\n' "brand"
  $PERFECTWORLD_SOURCE/bin/perfectworld-brain-cache get brand --project "$SLUG" 2>perfectworld-dev/null || printf '_(no brand digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
...
- If no DESIGN.md: "No design system found. Recommend running perfectworld-design-consultation first. Proceeding with universal design principles."
...
perfectworld designer is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the perfectworld designer. This is how we
...
`~/.perfectworld/projects/$SLUG/designs/` (user config directory, not project files).
Mockups are design artifacts that inform the plan, not code changes. The perfectworld
...
- `mkdir -p ~/.perfectworld/projects/$SLUG/designs/...`
...
```bash
eval "$($PERFECTWORLD_SOURCE/bin/perfectworld-slug 2>perfectworld-dev/null)"
_DESIGN_DIR="$HOME/.perfectworld/projects/$SLUG/designs/<screen-name>-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
fewer variants and benefits from sequential control. Note: perfectworld-design-shotgun uses
...
serves a single board at `/`, with reload at `perfectworld-api/reload` — only relevant
...
5. Reload the board in the user's browser (same tab) — the URL is per-board
   ...
   Under `--no-daemon` the reload endpoint is `perfectworld-api/reload` at the legacy
...
```bash
echo '{"approved_variant":"<V>","feedback":"<FB>","date":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","screen":"<SCREEN>","branch":"'$(git branch --show-current 2>perfectworld-dev/null)'"}' > "$_DESIGN_DIR/approved.json"
```
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The perfectworld designer isn't set up yet. Run `$D setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
For each finding: what's wrong, what will happen if it ships unresolved, and the specific fix. Be opinionated. No hedging." -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="high"' --enable web_search_cached < perfectworld-dev/null 2>"$TMPERR_DESIGN"
```
...
Re-run loop: invoke perfectworld-plan-design-review again → re-rate → sections at 8+ get a quick pass, sections below 8 get full treatment.
...
```bash
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output perfectworld-tmp/perfectworld-ideal-<dimension>.png
```
...
> **STOP.** Before running the 7 design passes, required outputs, and review report (only after Step 0 scope is agreed), Read `$PERFECTWORLD_SOURCE/plan-design-review/sections/review-sections.md` and execute it
