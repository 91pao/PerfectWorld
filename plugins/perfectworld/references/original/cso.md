...
preamble-tier: 2
version: 2.0.0
description: Chief Security Officer mode. (perfectworld)
allowed-tools:
  - Bash
  - Read
  - Grep
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"cso","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>perfectworld-dev/null)" 2>perfectworld-dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.perfectworld/analytics/skill-usage.jsonl 2>perfectworld-dev/null || true
fi
...
$PERFECTWORLD_SOURCE/bin/perfectworld-timeline-log '{"skill":"cso","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null &
_HAS_ROUTING="no"
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-question-log '{"skill":"cso","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>perfectworld-dev/null || true
```
...
# perfectworld-cso — Chief Security Officer Audit (v2)
...
## User-invocable
When the user types `perfectworld-cso`, run this skill.

## Arguments
- `perfectworld-cso` — full daily audit (all phases, 8/10 confidence gate)
- `perfectworld-cso --comprehensive` — monthly deep scan (all phases, 2/10 bar — surfaces more)
- `perfectworld-cso --infra` — infrastructure-only (Phases 0-6, 12-14)
- `perfectworld-cso --code` — code-only (Phases 0-1, 7, 9-11, 12-14)
- `perfectworld-cso --skills` — skill supply chain only (Phases 0, 8, 12-14)
- `perfectworld-cso --diff` — branch changes only (combinable with any above)
- `perfectworld-cso --supply-chain` — dependency audit only (Phases 0, 3, 12-14)
- `perfectworld-cso --owasp` — OWASP Top 10 only (Phases 0, 9, 12-14)
- `perfectworld-cso --scope auth` — focused audit on a specific domain
...
**Stack detection:**
```bash
ls package.json tsconfig.json 2>perfectworld-dev/null && echo "STACK: Node/TypeScript"
ls Gemfile 2>perfectworld-dev/null && echo "STACK: Ruby"
ls requirements.txt pyproject.toml setup.py 2>perfectworld-dev/null && echo "STACK: Python"
ls go.mod 2>perfectworld-dev/null && echo "STACK: Go"
ls Cargo.toml 2>perfectworld-dev/null && echo "STACK: Rust"
ls pom.xml build.gradle 2>perfectworld-dev/null && echo "STACK: JVM"
ls composer.json 2>perfectworld-dev/null && echo "STACK: PHP"
find . -maxdepth 1 \( -name '*.csproj' -o -name '*.sln' \) 2>perfectworld-dev/null | grep -q . && echo "STACK: .NET"
```

**Framework detection:**
```bash
grep -q "next" package.json 2>perfectworld-dev/null && echo "FRAMEWORK: Next.js"
grep -q "express" package.json 2>perfectworld-dev/null && echo "FRAMEWORK: Express"
grep -q "fastify" package.json 2>perfectworld-dev/null && echo "FRAMEWORK: Fastify"
grep -q "hono" package.json 2>perfectworld-dev/null && echo "FRAMEWORK: Hono"
grep -q "django" requirements.txt pyproject.toml 2>perfectworld-dev/null && echo "FRAMEWORK: Django"
grep -q "fastapi" requirements.txt pyproject.toml 2>perfectworld-dev/null && echo "FRAMEWORK: FastAPI"
grep -q "flask" requirements.txt pyproject.toml 2>perfectworld-dev/null && echo "FRAMEWORK: Flask"
grep -q "rails" Gemfile 2>perfectworld-dev/null && echo "FRAMEWORK: Rails"
grep -q "gin-gonic" go.mod 2>perfectworld-dev/null && echo "FRAMEWORK: Gin"
grep -q "spring-boot" pom.xml build.gradle 2>perfectworld-dev/null && echo "FRAMEWORK: Spring Boot"
grep -q "laravel" composer.json 2>perfectworld-dev/null && echo "FRAMEWORK: Laravel"
```
...
**Mental model:**
- Read AGENTS.md or repository guidance, README, key config files
...
{ find .github/workflows -maxdepth 1 \( -name '*.yml' -o -name '*.yaml' \) 2>perfectworld-dev/null; [ -f .gitlab-ci.yml ] && echo .gitlab-ci.yml; } | wc -l
find . -maxdepth 4 -name "Dockerfile*" -o -name "docker-compose*.yml" 2>perfectworld-dev/null
find . -maxdepth 4 -name "*.tf" -o -name "*.tfvars" -o -name "kustomization.yaml" 2>perfectworld-dev/null
ls .env .env.* 2>perfectworld-dev/null
```
...
> **STOP.** Before running the scope-dependent audit phases (Phases 2-11) selected by the resolved mode, after the Phase 0 stack detection and Phase 1 attack-surface census, Read `$PERFECTWORLD_SOURCE/cso/sections/audit-phases.md` and execute it
...
**Daily mode (default, `perfectworld-cso`):** 8/10 confidence gate. Zero noise. Only report what you're sure about.
...
**Comprehensive mode (`perfectworld-cso --comprehensive`):** 2/10 confidence gate. Filter true noise only (test fixtures, documentation, placeholders) but include anything that MIGHT be a real issue. Flag these as `TENTATIVE` to distinguish from confirmed findings.
...
22. Skill files that are part of perfectworld itself (trusted source)
...
**Trend Tracking:** If prior reports exist in `.perfectworld/security-reports/`:
```
...
```bash
mkdir -p .perfectworld/security-reports
```

Write findings to `.perfectworld/security-reports/{date}-{HHMMSS}.json` using this schema:
...
If `.perfectworld/` is not in `.gitignore`, note it in findings — security reports should stay local.
...
```bash
$PERFECTWORLD_SOURCE/bin/perfectworld-learnings-log '{"skill":"cso","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**This tool is not a substitute for a professional security audit.** perfectworld-cso is an AI-assisted
...
sensitive data, payments, or PII, engage a professional penetration testing firm. Use perfectworld-cso as
...
**Always include this disclaimer at the end of every perfectworld-cso report output.**