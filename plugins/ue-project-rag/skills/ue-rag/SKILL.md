...
3. Call `ue_rag_search` with a precise symbol, feature name, Gameplay Tag, config key, asset name, or error text. Search defaults to `mode: "and"`; use `mode: "or"` for broad discovery and `phrase: true` for an exact phrase. Keep `limit` at 8 or lower unless evidence is sparse.
