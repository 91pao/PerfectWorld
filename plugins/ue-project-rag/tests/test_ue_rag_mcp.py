...
class UeRagMcpTests(unittest.TestCase):
    ...
    def test_index_search_open_and_status(self):
        ...
        self.assertEqual(rebuilt["indexed_chunks"], 0)
        self.assertEqual(rebuilt["changed_files"], 0)
        (self.root / "Source" / "CombatComponent.cpp").write_text(
            "class UCombatComponent {}\nvoid UCombatComponent::ApplyCooldown() {}\nvoid UCombatComponent::ResetCooldown() {}\n",
            encoding="utf-8",
        )
        updated = SERVER.rebuild_index(str(self.root))
        self.assertEqual(updated["changed_files"], 1)
        self.assertGreater(updated["indexed_chunks"], 0)
        ...
        self.assertEqual(status["index_version"], "2")
        self.assertGreater(status["file_count"], 0)

    def test_search_modes_and_phrase(self):
        SERVER.rebuild_index(str(self.root))
        strict = SERVER.search_index(str(self.root), "CombatComponent Cooldown", mode="and")
        broad = SERVER.search_index(str(self.root), "CombatComponent Cooldown", mode="or")
        phrase = SERVER.search_index(str(self.root), "CombatComponent owns", phrase=True)
        self.assertGreaterEqual(broad["result_count"], strict["result_count"])
        self.assertEqual(strict["mode"], "and")
