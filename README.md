...
| 组件 | 当前版本 | 解决的问题 | 主要产物 |
| --- | --- | --- | --- |
| `ue-perfectworld` | 0.5.2 | UE 工程中的功能、Bug、资产、Blueprint、架构和代码风格如何找到正确边界 | S0-S6 阶段门禁工作流：规划与变更规格、实现、排查、审查、Blueprint 协作和开发文档 |
| `ue-project-rag` | 0.1.1 | 项目文件太多，单次搜索难以找到相关实现和配置 | 增量本地索引、AND/OR/短语检索、受限内容片段和索引状态 |
...
| 工作流 | 进入条件 | 主要处理 | 交付结果 |
| --- | --- | --- | --- |
| 规划与变更规格 | 新功能、架构或跨系统需求；用户需要完整方案但暂不写入文件 | 目标、现有能力、职责归属、依赖、风险和验证边界 | 可执行计划或可直接粘贴的变更规格 |
| 排查 | 编译错误、运行异常、回归或行为不一致 | 复现、调用链、数据流、生命周期和根因 | 根因、证据、修复范围和回归检查 |
| 直接实现 | 用户明确授权修改项目 | 最小范围修改 C++、Blueprint、配置或资产 | 改动文件与聚焦测试结果 |
| 审查 | 当前改动、PR 或 AI 生成代码 | 缺陷、职责重复、边界条件、兼容性、补丁应用完整性和测试缺口 | 按优先级排列的审查结论 |
