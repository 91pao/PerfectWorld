...
- **复杂度增长会重新审查**：方案新增文件、共享集成点、状态所有者、持久化路径、RPC、缓存、委托、定时器、管理器或配置源时，必须重新确认这些机制确有当前需求和项目能力依据。
- **交付时明确变更面**：按创建、替换、配置和仅复用列出文件、资产、表行、Tag、注册点与 Blueprint 绑定，区分真正需要修改的目标和项目已有依赖，并阻止未声明的共享框架改动。
